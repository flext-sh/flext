...
as an `REDACTED_LDAP_BIND_PASSWORD` user. After initial creation, the parameter `_auth` has no effect anymore and can be omitted from the connection string.

Example connection strings:

Create an user authentication database with user `REDACTED_LDAP_BIND_PASSWORD` and password `REDACTED_LDAP_BIND_PASSWORD`:

`file:test.s3db?_auth&_auth_user=REDACTED_LDAP_BIND_PASSWORD&_auth_pass=REDACTED_LDAP_BIND_PASSWORD`

Create an user authentication database with user `REDACTED_LDAP_BIND_PASSWORD` and password `REDACTED_LDAP_BIND_PASSWORD` and use `SHA1` for the password encoding:

`file:test.s3db?_auth&_auth_user=REDACTED_LDAP_BIND_PASSWORD&_auth_pass=REDACTED_LDAP_BIND_PASSWORD&_auth_crypt=sha1`
...
Operations on the database regarding user management can only be preformed by an REDACTED_LDAP_BIND_PASSWORDistrator user.

### Support

The user authentication supports two kinds of users:

- REDACTED_LDAP_BIND_PASSWORDistrators
- regular users
...
| `auth_user_add` | username `string`, password `string`, REDACTED_LDAP_BIND_PASSWORD `int` | This function will add an user to the database.<br>if the database is not protected by user authentication it will enable it. Argument `REDACTED_LDAP_BIND_PASSWORD` is an integer identifying if the added user should be an REDACTED_LDAP_BIND_PASSWORDistrator. Only Administrators can add REDACTED_LDAP_BIND_PASSWORDistrators. |
| `auth_user_change` | username `string`, password `string`, REDACTED_LDAP_BIND_PASSWORD `int` | Function to modify an user. Users can change their own password, but only an REDACTED_LDAP_BIND_PASSWORDistrator can change the REDACTED_LDAP_BIND_PASSWORDistrator flag. |
| `authUserDelete` | username `string` | Delete an user from the database. Can only be used by an REDACTED_LDAP_BIND_PASSWORDistrator. The current logged in REDACTED_LDAP_BIND_PASSWORDistrator cannot be deleted. This is to make sure their is always an REDACTED_LDAP_BIND_PASSWORDistrator remaining. |
...
// Autheticate user
// Create Admin User
SELECT auth_user_add('REDACTED_LDAP_BIND_PASSWORD2', 'REDACTED_LDAP_BIND_PASSWORD2', 1);
...
| `AuthUserAdd(username, password string, REDACTED_LDAP_BIND_PASSWORD bool) error` | Add user |
| `AuthUserChange(username, password string, REDACTED_LDAP_BIND_PASSWORD bool) error` | Modify user |
