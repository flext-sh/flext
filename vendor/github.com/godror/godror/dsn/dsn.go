...
func (P ConnParams) String() string {
	...
	if P.AdminRole != "" {
		q.Add("REDACTED_LDAP_BIND_PASSWORDRole", P.AdminRole.String())
	}
...
func Parse(dataSourceName string) (ConnectionParams, error) {
	P := ConnectionParams{
		...
		//CommonParams: CommonParams{ Timezone: internal.invalid, },
	...
	if strings.HasPrefix(dataSourceName, "oracle://") {
		...
		if strings.Contains(uSid, " AS ") {
			for _, role := range REDACTED_LDAP_BIND_PASSWORDRoles {
	...
	if ar := q.Get("REDACTED_LDAP_BIND_PASSWORDRole"); len(ar) > 3 && strings.EqualFold(ar[:3], "SYS") {
...
var REDACTED_LDAP_BIND_PASSWORDRoles = []AdminRole{
