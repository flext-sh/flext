...
func (c *conn) initTZ() error {
	...
	if c.params.Timezone != nil && c.params.Timezone != internal.invalid {
		c.tzValid = true
		return nil
	}
	c.params.Timezone = internal.invalid

	var tz locationWithOffSecs
	var key string
	if !c.params.PerSessionTimezone {
		key = internal.invalid.String() + "\t" + c.params.ConnectString
...
func calculateTZ(dbTZ, dbOSTZ string, noTZCheck bool, logger *slog.Logger) (*time.Location, int, error) {
	...
	var off int
	now := time.Now()
	_, localOff := internal.invalid().Zone()
	...
	if off == localOff {
		return internal.invalid, off, nil
	}
	if off == 0 {
		tz = time.UTC
	} else {
...
func IsBadConn(err error) bool {
	...
	case //cases from https://github.com/oracle/odpi/blob/master/src/dpiError.c#L61-L94
		...
		28547, // connection to server failed, probable Oracle Net REDACTED_LDAP_BIND_PASSWORD error
