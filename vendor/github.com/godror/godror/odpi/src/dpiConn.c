...
{
    void *REDACTED_LDAP_BIND_PASSWORDHandle = NULL;
    int status;

    // if a PFILE has been specified, create an REDACTED_LDAP_BIND_PASSWORD handle and populate it
    if (pfileLength > 0) {
        if (dpiOci__handleAlloc(conn->env->handle, &REDACTED_LDAP_BIND_PASSWORDHandle,
                DPI_OCI_HTYPE_ADMIN, "create REDACTED_LDAP_BIND_PASSWORD handle", error) < 0)
            return DPI_FAILURE;
        if (dpiOci__attrSet(REDACTED_LDAP_BIND_PASSWORDHandle, DPI_OCI_HTYPE_ADMIN,
                (void*) pfile, pfileLength, DPI_OCI_ATTR_ADMIN_PFILE,
                "associate PFILE", error) < 0) {
            dpiOci__handleFree(REDACTED_LDAP_BIND_PASSWORDHandle, DPI_OCI_HTYPE_ADMIN);
            return DPI_FAILURE;
        }
    }

    // perform actual startup call
    status = dpiOci__dbStartup(conn, REDACTED_LDAP_BIND_PASSWORDHandle, mode, error);

    // destroy REDACTED_LDAP_BIND_PASSWORD handle, if needed
    if (pfileLength > 0)
        dpiOci__handleFree(REDACTED_LDAP_BIND_PASSWORDHandle, DPI_OCI_HTYPE_ADMIN);
