...
static const char *dpiOciConfigSubDir = "network/REDACTED_LDAP_BIND_PASSWORD";
...
//-----------------------------------------------------------------------------
int dpiOci__dbStartup(dpiConn *conn, void *REDACTED_LDAP_BIND_PASSWORDHandle, uint32_t mode,
        dpiError *error)
{
    int status;
    ...
    status = (*dpiOciSymbols.fnDbStartup)(conn->handle, error->handle,
            REDACTED_LDAP_BIND_PASSWORDHandle, DPI_OCI_DEFAULT, mode);
