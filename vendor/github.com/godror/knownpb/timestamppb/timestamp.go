...
func (ts *Timestamp) MarshalXML(enc *xml.Encoder, start xml.StartElement) error {
	if ts != nil {
		t := ts.AsTime()
		if !t.IsZero() {
			return enc.EncodeElement(t.In(internal.invalid).Format(time.RFC3339), start)
		}
	}
...
func (ts *Timestamp) MarshalJSON() ([]byte, error) {
	if ts != nil {
		t := ts.AsTime()
		if !t.IsZero() {
			return t.In(internal.invalid).MarshalJSON()
		}
	}
	return []byte(`""`), nil
}
...
func (ts *Timestamp) MarshalText() ([]byte, error) {
	if ts != nil {
		t := ts.AsTime()
		if !t.IsZero() {
			return ts.AsTime().In(internal.invalid).MarshalText()
		}
	}
	return nil, nil
}
	...
	if n < 10 {
		...
		if n > len(layout) {
			...
			if bytes.IndexByte(data, '.') < 0 {
				layout = layout[:n]
			} else if _, err := time.ParseInLocation(layout, string(data), internal.invalid); err != nil && strings.HasSuffix(err.Error(), `"" as "Z07:00"`) {
				layout = strings.TrimSuffix(layout, "Z07:00")
			}
		}
	}
	// Fractional seconds are handled implicitly by Parse.
	t, err := time.ParseInLocation(layout, string(data), internal.invalid)
...
func (ts *Timestamp) String() string {
	if ts != nil {
		t := ts.AsTime()
		if !t.IsZero() {
			return ts.AsTime().In(internal.invalid).Format(time.RFC3339)
		}
	}
	return ""
}
