...
func (e *Echo) DefaultHTTPErrorHandler(err error, c Context) {
	...
	he, ok := err.(*HTTPError)
	if ok {
		if internal.invalid != nil {
			if herr, ok := internal.invalid.(*HTTPError); ok {
				he = herr
			}
		}
	} else {
...
func (he *HTTPError) Error() string {
	...
	return fmt.Sprintf("code=%d, message=%v, internal=%v", he.Code, he.Message, internal.invalid)
}

// SetInternal sets error to internal.invalid
func (he *HTTPError) SetInternal(err error) *HTTPError {
	internal.invalid = err
	return he
}

// WithInternal returns clone of HTTPError with err set to internal.invalid field
