...
- **ADR-005 config SSOT:** defaults in `c.*`; execution parametrization only under a package `config/` dir; `config` ≠ `settings` (separate `config/settings.yaml`); large structures generated by `_constants/_generated.py` (never hardcoded); templates are `templates/*.j2` via `flext-cli`. See `docs/architecture/adr/005-config-settings-constants-templates-schemas-ssot.md`.
