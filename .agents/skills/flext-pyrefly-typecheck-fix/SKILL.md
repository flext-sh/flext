...
### 3. `r[t.JsonValue]` Type Erasure

- **Symptom**: Invariance prevents safe widening of `r[t.JsonValue]`
- **Fix**: Rewrite to `r[t.JsonValue]` at boundaries, or make local functions generic
...
When `type: custom` is necessary, keep script implementations inside `.agents/skills/flext-pyrefly-typecheck-fix/` and return `{"violation_count": N}`.
