...
- `**kwargs: object` or `dict[str, object]` in runtime code is forbidden; use canonical JSON-safe `t.*` contracts.
...
class FlextDemoParseJsonMixin:
    ...
    def parse_json(self, request: t.JsonPayload) -> p.Result[str]:
