...
class FlextTaskOrchestrationConstants(FlextConstants):
    ...
    class Configuration:
        ...
        DEFAULT_MIN_ESTIMATION_HOURS = 0.5
        DEFAULT_MAX_ESTIMATION_HOURS = 40.0
        DEFAULT_REQUIRE_QA = True
    ...
    class Messages:
        ...
        TASK_ESTIMATION_BELOW_MINIMUM = "Task '{title}' estimation below minimum: {hours}"
        TASK_ESTIMATION_ABOVE_MAXIMUM = "Task '{title}' estimation above maximum: {hours}"
        INVALID_PROGRESS_PERCENTAGE = "Progress percentage must be between 0 and 100"
        CIRCULAR_DEPENDENCY_DETECTED = "Circular dependency detected involving task {task_id}"
        TASK_DEPENDENCY_NON_EXISTENT = "Task {task_id} has dependency on non-existent task {dep_task_id}"
