...
class TestTaskOrchestrator:
    ...
    def test_focus_filtering(self) -> None:
        ...
        security_related = [
            req for req in requirements if "security" in req.get("title", "").lower()
        ]
...
class TestDependencyAnalyzer:
    ...
    def test_circular_dependency_detection(self) -> None:
        ...
        from flext.task_orchestration.models import TaskDependency
        ...
        _updated_tasks, conflicts, _parallel_groups = result.unwrap()

        # Should detect circular dependency
        circular_conflicts = [
            c for c in conflicts if c.get("type") == "circular_dependency"
        ]
...
class TestTaskModel:
    ...
    def test_task_dependencies(self) -> None:
        ...
        task = Task(
            ...
            description="Test task description",
            dependencies=[
                TaskDependency(task_id="TASK-123456", dependency_type="blocks")
            ],
        )
        ...
        # Check if blocked (direct check for blocking dependencies)
        has_blocking_deps = any(
            dep.dependency_type == "blocks" for dep in task.dependencies
        )
        assert has_blocking_deps

        # Check if can start (not blocked and status is TODO)
        can_start = not has_blocking_deps and task.status == TaskStatus.TODO
    ...
    def test_task_validation(self) -> None:
        ...
        # Test valid task
        task = Task(
            title="Valid Task", description="Valid description", estimated_hours=5.0
        )
...
class TestTaskOrchestrationConfig:
    ...
    def test_custom_config(self) -> None:
        ...
        config = TaskOrchestrationConfig(
            max_agents=5, parallel_tasks=3, focus_area="security", auto_assign=False
        )
    ...
    def test_config_validation(self) -> None:
        ...
        # Test valid ranges
        config = TaskOrchestrationConfig(
            max_agents=5, parallel_tasks=10, max_task_duration_days=60
        )
        assert config.max_agents == 5
