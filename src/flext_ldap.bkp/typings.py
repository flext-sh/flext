...
class FlextLdapTypes(FlextTypes):
    ...
    class Entry:
        ...
        type Instance = FlextLdapProtocols.Entry.EntryProtocol
        type Handler[T] = Callable[[Instance], object]
    ...
    # ═══════════════════════════════════════════════════════════════════
    # DOMAIN-SPECIFIC TYPE ALIASES (PEP 695 syntax)
    # ═══════════════════════════════════════════════════════════════════

    class LdapEntry:
        """Entry-related type aliases."""

        # Tipos básicos para entries
        type Instance = object  # Placeholder - would be EntryProtocol if available
        type Collection = Sequence[Instance]
        type Mapping = Mapping[str, Instance]

        # Tipos com generics usando TypeVars do módulo
        type Handler[T] = Callable[[FlextLdapTypes.LdapEntry.Instance], object]
        type Transformer = Callable[[Instance], Instance]

    class LdapConfig:
        """Configuration-related type aliases."""

        # Tipos básicos para config
        type ValueType = str | int | float | bool
        type SettingsType = Mapping[str, ValueType]

        # Tipos específicos com composição
        type DomainSettings = Mapping[str, ValueType]
    ...
    type SearchResult = Sequence[LdapEntry.Instance]
