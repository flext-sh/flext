...
from .version import VersionAnalyzer
...
class ConflictAnalyzer:
    ...
    def _extract_project_dependencies(
        ...
        try:
            ...
            print_colored(f"  ⚠️ Error reading {pyproject_path}: {e}")
            return {}
        else:
            ...
            if tool_section is None:
                return deps
            main_deps = tool_section.get("poetry", {}).get("dependencies", {})
    ...
    def _extract_version_string(self, spec: str | FlextTypes.Core.Dict | object) -> str:
        ...
        if isinstance(spec, dict):
            version: object = spec.get("version", "*")
            return str(version)
            ...
            "affected_projects": len(
                {
                    ...
                    if "blocking_projects" in blocker_data
                    for project in (
    ...
    def _format_conflict_entry(
        ...
        if isinstance(projects_data, dict):
            lines.extend(
                ...
                if isinstance(project, str) and isinstance(version, str)
            )
