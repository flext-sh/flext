...
class LockConsistencyAnalyzer:
    ...
    class ProjectLockInfo:
        ...
        lock_path: Path
        exists: bool
        packages: dict[str, LockConsistencyAnalyzer.LockFileEntry]
    ...
    def __init__(self) -> None:
        """Initialize analyzer."""
        self.project_locks: dict[str, LockConsistencyAnalyzer.ProjectLockInfo] = {}
    ...
    def analyze_workspace(
        ...
        try:
            ...
            print_colored(f"  📁 Found {len(projects)} projects")
    ...
    def _load_project_lock(self, project_path: Path) -> None:
        ...
        if not lock_path.exists():
            self.project_locks[project_name] = self.ProjectLockInfo(
        ...
        try:
            ...
            for package_data in data.get("package", []):
                ...
                if files and isinstance(files[0], dict):
                    hash_value = str(files[0].get("hash", ""))

                # Extract package dependencies
                dependencies = package_data.get("dependencies", {})

                packages[name] = self.LockFileEntry(
                    name=name,
                    version=version,
                    hash=hash_value,
            ...
            # Convert to the expected type for the dataclass
            packages_converted: dict[str, LockConsistencyAnalyzer.LockFileEntry] = {}
            for name, entry in packages.items():
                packages_converted[name] = LockConsistencyAnalyzer.LockFileEntry(
                    name=entry.name,
            ...
            self.project_locks[project_name] = self.ProjectLockInfo(
                ...
                lock_path=lock_path,
                exists=True,
                packages=packages_converted,
                lock_version=lock_version,
                python_versions=[python_versions] if python_versions else [],
            )

            print_colored(f"    ✅ {project_name}: {len(packages)} packages")
        except (OSError, tomllib.TOMLDecodeError, KeyError) as e:
            print_colored(f"    ❌ {project_name}: Error reading poetry.lock - {e}")
            self.project_locks[project_name] = self.ProjectLockInfo(
                project_name=project_name,
                lock_path=lock_path,
                exists=False,
    ...
    def _analyze_inconsistencies(self) -> None:
        ...
        if len(self.project_locks) < MIN_VALID_PROJECTS:
            print_colored("  ⚠️ Less than 2 projects with valid poetry.lock files")
            return
        ...
        print_colored(f"  📦 Analyzing {len(all_packages)} unique packages")
    ...
    def _analyze_package_consistency(
        self,
        package: str,
        projects: dict[str, LockConsistencyAnalyzer.ProjectLockInfo],
    ) -> None:
    ...
    def _categorize_inconsistencies(self) -> dict[str, list[LockInconsistency]]:
        ...
        if total > 0:
            ...
            print_colored(f"  🔴 Critical: {len(categories['critical'])}")
            print_colored(f"  🟡 Warnings: {len(categories['warning'])}")
            print_colored(f"  [INFO] Info: {len(categories['info'])}")
        else:
    ...
    def get_workspace_summary(
        ...
        counts, package statistics, and inconsistency summaries.
        """
        ...
        try:
            ...
            return FlextResult["LockConsistencyAnalyzer.WorkspaceSummary"].ok(summary)
    ...
    def print_detailed_report(
        ...
        for severity, inconsistencies in categories.items():
            ...
            print_colored(f"\n{severity_label} ({len(inconsistencies)}):")
...
class LockInconsistency(FlextModels.Value):
    """Detailed inconsistency detection with severity classification."""
...
class WorkspaceSummary(FlextModels.Value):
    """Workspace summary for lock consistency analysis."""

    total_projects: int = Field(..., description="Total number of projects")
    projects_with_lock: int = Field(..., description="Projects with lock files")
    projects_without_lock: int = Field(..., description="Projects without lock files")
    total_inconsistencies: int = Field(..., description="Total inconsistencies found")
    critical_inconsistencies: int = Field(..., description="Critical inconsistencies")
