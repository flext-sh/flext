...
class QualityBridge:
    ...
    def analyze_project_via_service(
        ...
        try:
            ...
            # Import flext-quality service for delegation
            with contextlib.suppress(ImportError):
                ...
                result = api.analyze_project(str(project_path))
                
                if result.success:
                    return FlextResult.ok(result.data)
                else:
                    return FlextResult.fail(f"Quality analysis failed: {result.error}")
                    
            # Fallback when flext-quality service not available
            return FlextResult.fail(
                "flext-quality service not available for delegation. "
                "Install flext-quality package for full quality analysis capabilities."
            )
    ...
    def validate_workspace_quality(self) -> FlextResult[dict[str, Any]]:
        ...
        try:
            ...
            for project_dir in python_projects:
                ...
                if analysis_result.success and analysis_result.data:
                    ...
                    project_score = project_data.get("overall_score", 0.0)
                    project_issues = len(project_data.get("issues", []))
                    
                    workspace_results["project_results"][project_name] = {
    ...
    def get_service_status(self) -> FlextResult[dict[str, Any]]:
        ...
        """
        try:
            # Check if flext-quality service is available for delegation
            with contextlib.suppress(ImportError):
                from flext_quality import QualityAPI
            ...
            return FlextResult.ok({
                ...
                "workspace_root": str(self.workspace_root),
                "error": "flext-quality package not installed"
            })
