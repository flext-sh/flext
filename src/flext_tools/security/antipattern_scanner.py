...
class AntipatternScanner:
    ...
    def _collect_python_files(self) -> FlextResult[list[Path]]:
        ...
        try:
            ...
            return FlextResult.fail(f"Failed to collect Python files: {e}")
    ...
    def _should_include_file(self, file_path: Path) -> bool:
        ...
        if not self.config.include_dependencies and ".venv" in file_str:
            return False
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ExceptHandler):
                    # Check for dangerous patterns in exception handlers
                    if self._is_dangerous_except_handler(node):
                        violations.append(SecurityViolation(
                            file_path=str(file_path),
    ...
    def _is_dangerous_except_handler(self, node: ast.ExceptHandler) -> bool:
        ...
        for stmt in node.body:
            if isinstance(stmt, ast.Return) and stmt.value:
                if isinstance(stmt.value, (ast.Constant, ast.NameConstant, ast.Num, ast.Str)):
                    return True
    ...
    def _get_risk_color(self, risk: str) -> Colors:
...
def create_security_scanner(
    ...
    """Factory function to create security scanner with common configurations.
    ...
    try:
        ...
        scanner = AntipatternScanner(config)
        return FlextResult.ok(scanner)
        
    except Exception as e:
        return FlextResult.fail(f"Failed to create security scanner: {e}")
...
def scan_flext_ecosystem(
    ...
    """Convenience function to scan entire FLEXT ecosystem.
    ...
    if not existing_paths:
        return FlextResult.fail("No valid FLEXT project paths found")
    ...
    if not scanner_result.success:
        return FlextResult.fail(f"Scanner creation failed: {scanner_result.error}")
    
    scanner = scanner_result.data
    if not scanner:
        return FlextResult.fail("Scanner creation returned None")
