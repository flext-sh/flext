...
class PythonImportDiscovery:
    ...
    def __init__(self, stdlib_modules: set[str]) -> None:
        """Initialize discovery with stdlib modules."""
    ...
    def discover(self, project_path: Path, installed: set[str]) -> dict[str, set[str]]:
        ...
        # Inicializa estrutura de dependências
    ...
    def _categorize_imports(
        ...
        """Categoriza imports em runtime/test e adiciona às dependências."""
        for import_name in file_imports:
            ...
            package_name = import_name  # Inicializa com o nome original
            if import_name in self.submodules_mapping:
                ...
                package_name = parent_package  # Usa o pacote pai
    ...
    def _extract_imports(self, file_path: Path) -> set[str]:
        ...
        try:
            ...
            print_colored(
                f"    ⚠️ Erro ao analisar {file_path.name}: {str(e)[:50]}",
                Colors.YELLOW,
            )
