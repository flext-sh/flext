...
class DependencyDiscovery:
    ...
    def discover_project_dependencies(
        ...
        if self.resolve_transitive and hasattr(self, "transitive_resolver"):
            ...
            print_colored(
                f"  🔗 Dependências transitivas: {len(available_transitive)}",
                Colors.CYAN,
            )
            ...
            for category, deps in result.items():
                ...
                if removed_count > 0:
                    print_colored(
                        f"    ✓ {removed_count} transitivas removidas de {category}",
                        Colors.GREEN,
                    )
        ...
        try:
            ...
            if pep621_deps:
                ...
                for dep_spec in pep621_deps:
                    # Extrai nome do pacote da especificação
                    # (ex: "pydantic>=2.0.0" -> "pydantic")
