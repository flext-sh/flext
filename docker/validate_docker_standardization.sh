...
# ============================================================================
# CHECK 6: Verify tk availability
# ============================================================================
echo -e "${BLUE}[6/8]${NC} Verifying tk availability..."
if python3 -c "from flext_tests import tk; print('OK')" 2>/dev/null | grep -q "OK"; then
	echo -e "${GREEN}✅ PASSED${NC}: tk is importable"
else
	echo -e "${RED}❌ FAILED${NC}: Cannot import tk from flext_tests"
	((ERRORS++))
fi
echo ""
...
if [ "$DOCKER_SCRIPTS" -gt 0 ]; then
	...
	echo -e "    ${YELLOW}Consider using tk API instead${NC}"
	((WARNINGS++))
else
