...
# ============================================================================
# CHECK 6: Verify TestsFlextDocker availability
# ============================================================================
echo -e "${BLUE}[6/8]${NC} Verifying TestsFlextDocker availability..."
if python3 -c "from flext_tests import TestsFlextDocker; print('OK')" 2>/dev/null | grep -q "OK"; then
	echo -e "${GREEN}✅ PASSED${NC}: TestsFlextDocker is importable"
else
	echo -e "${RED}❌ FAILED${NC}: Cannot import TestsFlextDocker from flext_tests"
	((ERRORS++))
fi
echo ""
...
if [ "$DOCKER_SCRIPTS" -gt 0 ]; then
	...
	echo -e "    ${YELLOW}Consider using TestsFlextDocker API instead${NC}"
	((WARNINGS++))
else
