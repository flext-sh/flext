...
class TempFileCleanupModule(CustomFixModule):
        ...
        if is_temp_file and file_age < cutoff_date:
            ...
            issues.append(
                Issue(
                    ...
                    column=1,
                    code="TEMP001",
                    message=f"Temporary file older than {
                        self.max_age_days} days (age: {age_days} days)",
    ...
    def cleanup_temp_files(self, workspace_path: Path = None) -> bool:
        ...
        if self.verbose:
            ...
            self.console.print(
                f"[yellow]Maximum age: {
                    self.max_age_days} days[/yellow]")
        ...
        if not old_files:
            if self.verbose:
                self.console.print(
                    "[green]No old temporary files found[/green]")
            return True

        if self.verbose:
            self.console.print(
                f"[yellow]Found {
                    len(old_files)} old temporary files[/yellow]")
        ...
        for file_path in old_files:
            try:
                ...
                if self.verbose:
                    self.console.print(
                        f"[cyan]File: {
                            file_path.relative_to(workspace_path)} (age: {age_days} days, purpose: {
                            metadata.get(
                                'purpose',
                                'Not specified')})[/cyan]")
        ...
        if self.verbose:
            ...
            self.console.print(
                f"[bold green]{action} {
                    len(old_files)} temporary files[/bold green]")
