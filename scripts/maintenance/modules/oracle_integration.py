...
class OracleIntegrationModule(CustomFixModule):
    ...
    def test_oracle_connection(self, config_type: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if config_type == "oic":
                ...
                return False, f"OIC authentication failed: {response.status_code}"
    ...
    def _get_key_columns(self, table_name: str) -> list[str]:
        ...
        for _mapping_name, mapping in self.WMS_TABLE_MAPPINGS.items():
            if (
                ...
                or mapping["source_table"] == table_name
            ):
        ...
        try:
            ...
            if results["success"] and config.get("validate_schema", True):
                ...
                for table in config.get("tables", []):
                    success, schema_info = self.validate_oracle_schema("wms", table)
                    results["steps"].append(
                        {
            ...
            if results["success"] and config.get("extract_data", True):
                if self.verbose:
                    self.console.print("[blue]Extracting data from Oracle[/blue]")

                for table in config.get("source_tables", []):
                    conditions = config.get("extraction_conditions", {}).get(table)
                    ...
                    if success:
                        ...
                        results["success"] = False
                        results["errors"].append(f"Data extraction failed for {table}")
            ...
            if results["success"] and config.get("load_data", True):
                ...
                for table, data in results.get("extracted_data", {}).items():
                    target_table = config.get("table_mappings", {}).get(table, table)
            ...
            for table_name, table_data in data.items():
                ...
                for record in table_data:
                    ...
                    if "field_mappings" in table_transforms:
                        for old_field, new_field in table_transforms[
                            "field_mappings"
                        ].items():
                    ...
                    if "type_conversions" in table_transforms:
                        for field, target_type in table_transforms[
                            "type_conversions"
                        ].items():
                    ...
                    if "default_values" in table_transforms:
                        for field, default_value in table_transforms[
                            "default_values"
                        ].items():
        ...
        if file_path.suffix == ".py":
            ...
            for i, line in enumerate(lines, 1):
                ...
                if any(
                    ...
                    for pattern in ["cx_Oracle.connect(", "oracledb.connect("]
                ):
                ...
                if "cursor.execute(" in line_stripped:
                    ...
                    if not any(
                        "try:" in ctx_line or "except" in ctx_line
                ...
                if (
                    ...
                    and "FROM" in line_stripped.upper()
                ):
                    issues.append(
                        Issue(
                            line=i,
        ...
        for issue in issues:
            if issue.code == "ORACLE001":  # Fix hardcoded connections
                ...
                if line_idx < len(lines):
                    ...
                    lines.insert(line_idx + 1, " " * (indent + 4) + line.strip())
                    lines.insert(line_idx + 2, " " * indent + "except Exception as e:")
                    lines.insert(
                        line_idx + 3,
                        " " * (indent + 4)
    ...
    def run_oracle_integration_workflow(
        ...
        for project_dir in workspace_path.iterdir():
            if project_dir.is_dir() and not project_dir.name.startswith("."):
                if any(
                    oracle_keyword in project_dir.name.lower()
    ...
    def _show_integration_summary(self, results: dict[str, Any]) -> None:
        ...
        # Summary panel
        success_rate = (
            (results["successful_integrations"] / results["total_projects"] * 100)
