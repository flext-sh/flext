...
class ValidationResults:
    ...
    errors_by_domain: Dict[str, List[str]] = field(default_factory=lambda: defaultdict(list))
...
class LinkValidator:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.user_agent = self.config.get("user_agent",
                                        "FLEXT-Docs-Validator/1.0 (https://github.com/flext-sh/flext)")
        ...
        if self.cache_file.exists():
            try:
                with open(self.cache_file, 'r') as f:
                    ...
                    return {
                        url: result for url, result in data.items()
                        if current_time - result.get("timestamp", 0) < self.cache_duration
                    }
    ...
    async def validate_link_async(self, session: aiohttp.ClientSession,
                                url: str, retry_count: int = 0) -> LinkResult:
        ...
        try:
            ...
            async with session.head(url, headers=headers,
                                  allow_redirects=True, timeout=self.timeout) as response:
                ...
                if response.status == 405:  # Method not allowed, try GET
                    async with session.get(url, headers=headers,
                                         allow_redirects=True, timeout=self.timeout) as get_response:
                        response_time = time.time() - start_time
                        redirect_url = str(get_response.url) if str(get_response.url) != url else None
    ...
    async def validate_links_batch(self, links: List[str]) -> List[LinkResult]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    processed_results.append(LinkResult(
    ...
    def validate_internal_links(self, doc_files: List[Path]) -> Dict[str, List[str]]:
    ...
    def generate_report(self, validation_results: ValidationResults,
                       internal_issues: Dict[str, List[str]]) -> str:
...
"""
        ...
        if validation_results.broken_links > 0:
            report += f"- **Fix {validation_results.broken_links} broken external links**\n"
        ...
        if validation_results.total_links > 0:
            health_score = validation_results.valid_links / validation_results.total_links
...
def main():
    ...
    parser.add_argument("--external-only", action="store_true", help="Validate only external links")
    parser.add_argument("--internal-only", action="store_true", help="Validate only internal links")
    ...
    # Discover files
    if args.files:
        doc_files = [Path(f) for f in args.files if Path(f).exists() and f.endswith(('.md', '.mdx'))]
    else:
        doc_files = []
    ...
    if not args.internal_only:
        ...
        external_results = asyncio.run(validator.validate_links_batch(list(all_external_links)))
