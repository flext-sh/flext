...
def run_command(cmd: str, env_file: str = ".env.test") -> None:
    ...
    project_dir = Path(__file__).parent / "client-b_oic_wms"
...
def main() -> None:
    ...
    # Copy test env file
    env_test = Path("client-b_oic_wms/.env.test")
    env_file = Path("client-b_oic_wms/.env")
    ...
    cli_file = Path("client-b_oic_wms/src/gn_oic_wms_db/cli.py")
    if cli_file.exists():
