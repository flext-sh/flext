...
class FalsePositiveAuditor:
    ...
    def audit_project_dependencies(self, project_path: Path) -> dict[str, list[dict]]:
        """
    ...
    def _print_audit_results(self, analysis: dict[str, list[dict]]):
...
def audit_workspace():
    ...
    for category, items in global_analysis.items():
        if not items:
            continue
