    ...
    for class_name, class_info in inventory["classes"].items():
        if class_name.startswith("Flx"):
            for method in class_info["methods"]:
                if not method["name"].startswith("_") and not method["name"].startswith("flx_"):
                    if method["name"] not in {"__init__", "__str__", "__repr__", "__eq__", "__hash__"}:
                        inventory["missing_flx_prefixes"]["functions"].append({
...
def main() -> None:
    ...
    print(f"- Flx-prefixed classes: {sum(1 for c in inventory['classes'].values() if c['is_flx_prefixed'])}")
    print(f"- Total functions: {len(inventory['functions'])}")
    print(f"- flx-prefixed functions: {sum(1 for f in inventory['functions'].values() if f['is_flx_prefixed'])}")
    ...
    print("\nMissing prefixes:")
    print(f"- Classes without Flx prefix: {len(inventory['missing_flx_prefixes']['classes'])}")
    print(f"- Functions/methods without flx_ prefix: {len(inventory['missing_flx_prefixes']['functions'])}")
