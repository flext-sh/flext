...
class ClaudeDocumentationValidator:
    ...
    def __init__(self, workspace_root: str = "/home/marlonsc/pyauto"):
        ...
        self.global_claude_local = Path("/home/marlonsc/CLAUDE.local.md")
        self.workspace_claude_md = self.workspace_root / "CLAUDE.md"
        self.workspace_claude_local = self.workspace_root / "CLAUDE.local.md"
    ...
    def validate_venv_standardization(self) -> dict[str, Any]:
        ...
        # Check for venv documentation in project CLAUDE.local.md files
        for project_dir in self._get_project_directories():
    ...
    def validate_env_security(self) -> dict[str, Any]:
        ...
        for project_dir in self._get_project_directories():
            ...
            # Check for .env security documentation in CLAUDE.local.md
            claude_local = project_dir / "CLAUDE.local.md"
            if claude_local.exists():
                try:
...
5. Read project CLAUDE.local.md for specific issues
...
cat CLAUDE.local.md  # Read project-specific documentation
```
"""
...
"""
    ...
    def _get_single_project_context(self, project_name: str):
        ...
        if claude_local.exists():
            ...
            elif "algar-" in project_name or "gruponos-" in project_name:
    ...
    def _generate_recommendations(self, validation_results):
        ...
        if missing_projects:
            recommendations.append(
                f"Create CLAUDE.local.md for {len(missing_projects)} projects: {', '.join(missing_projects[:3])}..."
            )
