...
[ alt_names ]
DNS.1 = staging.flext.local
DNS.2 = staging-admin.flext.local
DNS.3 = localhost
DNS.4 = 127.0.0.1
IP.1 = 127.0.0.1
IP.2 = ::1
...
server {{
    ...
    server_name staging.flext.local;
...
server {{
    listen 443 ssl http2;
    server_name staging-admin.flext.local;
...
server {{
    listen 80;
    server_name staging.flext.local staging-admin.flext.local;
...
127.0.0.1    staging.flext.local
127.0.0.1    staging-admin.flext.local
"""
    ...
    print("    127.0.0.1    staging.flext.local")
    print("    127.0.0.1    staging-admin.flext.local")
...
# Test API endpoint
echo "Testing API (https://staging.flext.local)..."
curl -k -s -o /dev/null -w "Status: %{{http_code}}\\n" https://staging.flext.local/health || echo "❌ API not responding"

# Test Web endpoint
echo "Testing Web (https://staging-admin.flext.local)..."
curl -k -s -o /dev/null -w "Status: %{{http_code}}\\n" https://staging-admin.flext.local/ || echo "❌ Web not responding"
...
echo "4. Test with: curl -k https://staging.flext.local/health"
"""
...
def main():
    ...
    print("  🌐 Domains: staging.flext.local, staging-admin.flext.local")
