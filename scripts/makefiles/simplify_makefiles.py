...
""",
        "algar-oud-mig": """
# ALGAR OUD Migration project
PROJECT_NAME := algar_oud_mig
SRC_MODULE := algar_oud_mig

# ALGAR-specific commands
process-ldif: poetry-check ## Process LDIF files
	@echo "$(BLUE)🔄 Processing LDIF files...$(NC)"
	@$(POETRY_RUN) python -m algar_oud_mig.cli process

validate-ldap: poetry-check ## Validate LDAP connection
	@echo "$(BLUE)🔍 Validating LDAP connection...$(NC)"
	@$(POETRY_RUN) python -m algar_oud_mig.cli validate
""",
    }
