...
class StyleCheckResults:
    ...
    total_files: int = 0
    total_issues: int = 0
    issues_by_type: Dict[str, List[StyleIssue]] = field(default_factory=lambda: defaultdict(list))
    issues_by_severity: Dict[str, List[StyleIssue]] = field(default_factory=lambda: defaultdict(list))
...
class DocumentationStyleChecker:
    ...
    def check_line(self, line: str, line_num: int, all_lines: List[str],
                  file_path: Path) -> List[StyleIssue]:
        """Check a single line for style issues."""
        issues = []

        # Check line length
        max_length = self.style_rules.get("max_line_length", 120)
        if len(line) > max_length and not self.is_code_block_line(line, all_lines, line_num - 1):
            issues.append(StyleIssue(
                file_path=file_path,
                ...
                suggestion=f"Break line into multiple lines or shorten content",
        ...
        if self.is_list_item(line):
            issues.extend(self.check_list_formatting(line, line_num, all_lines, file_path))
        ...
        if '|' in line and self.style_rules.get("table_alignment", True):
            issues.extend(self.check_table_alignment(line, line_num, all_lines, file_path))

        return issues

    def check_heading(self, line: str, line_num: int, file_path: Path) -> List[StyleIssue]:
        """Check heading formatting."""
        issues = []
    ...
    def check_list_formatting(self, line: str, line_num: int,
                            all_lines: List[str], file_path: Path) -> List[StyleIssue]:
    ...
    def check_code_block(self, line: str, line_num: int,
                        all_lines: List[str], file_path: Path) -> List[StyleIssue]:
    ...
    def check_image_alt_text(self, line: str, line_num: int, file_path: Path) -> List[StyleIssue]:
        """Check image alt text."""
        issues = []
    ...
    def check_table_alignment(self, line: str, line_num: int,
                            all_lines: List[str], file_path: Path) -> List[StyleIssue]:
        ...
        if '|' in line and not line.strip().startswith('|'):
            ...
            if pipe_count > 1:
                ...
                if ('|' in prev_line or '|' in next_line):
                    ...
                    if prev_pipes != pipe_count and next_pipes != pipe_count:
                        issues.append(StyleIssue(
                            file_path=file_path,
                            line_number=line_num,
    ...
    def check_file_structure(self, content: str, lines: List[str], file_path: Path) -> List[StyleIssue]:
        """Check overall file structure."""
        issues = []
        ...
        if len(headings) < min_headings and len(lines) > 50:  # Only check for substantial files
            issues.append(StyleIssue(
                file_path=file_path,
                line_number=1,
    ...
    def check_heading_hierarchy(self, lines: List[str], file_path: Path) -> List[StyleIssue]:
    ...
    def is_code_block_line(self, line: str, all_lines: List[str], line_index: int) -> bool:
...
"""
        ...
        for issue_type, issues in results.issues_by_type.items():
            if issues:
                ...
                status_icon = {"critical": "❌", "high": "🔴", "medium": "🟡", "low": "🟢"}.get(severity, "⚪")
...
"""
        ...
        if results.total_issues > 0:
            report += f"- **Run automated fixes for {results.total_issues} total issues**\n"
            report += "- **Consider updating style configuration** for team preferences\n"
            report += "- **Set up pre-commit hooks** to prevent style issues\n"

        # Quality score
        if results.total_files > 0:
            quality_score = 1.0 - (results.total_issues / (results.total_files * 10))  # Rough heuristic
...
def discover_docs(base_path: Path = None) -> List[Path]:
...
def main():
    ...
    parser.add_argument("files", nargs="*", help="Specific files to check (default: all docs)")
    ...
    # Summary
    if results.total_issues > 0:
        print(f"⚠️ Found {results.total_issues} style issues across {len(results.files_with_issues)} files")
