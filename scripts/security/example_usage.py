...
def example_basic_scan() -> None:
    ...
    if result.success:
        ...
        print_colored(f"✅ Scan completed: {len(violations)} violations found", Colors.GREEN)
...
def example_advanced_configuration() -> None:
    ...
    if not validation_result.success:
        print_colored(f"❌ Invalid configuration: {validation_result.error}", Colors.RED)
        return
    ...
    if result.success:
        violations = result.data or []
        print_colored(f"✅ Advanced scan completed: {len(violations)} violations found", Colors.GREEN)
...
def example_ecosystem_scan() -> None:
    ...
    if result.success:
        violations = result.data or []
        print_colored(f"✅ Ecosystem scan completed: {len(violations)} violations found", Colors.GREEN)
...
def example_custom_reporting() -> None:
    ...
    top_files = sorted(file_violations.items(), key=lambda x: len(x[1]), reverse=True)[:3]
    ...
    for file_name, file_viols in top_files:
        ...
        types_in_file = set(v.violation_type.value for v in file_viols)
    ...
    for risk, count in sorted(risk_counts.items()):
        color = Colors.RED if risk == "CRITICAL" else Colors.YELLOW if risk == "HIGH" else Colors.GREEN
...
def example_ci_cd_integration() -> None:
    ...
    if not scanner_result.success:
        print_colored(f"❌ CI/CD scanner creation failed: {scanner_result.error}", Colors.RED)
        return
    ...
    if critical_violations:
        print_colored(f"🚨 BUILD SHOULD FAIL: {len(critical_violations)} critical violations", Colors.RED)
        
        print_colored("Critical violations that must be fixed:", Colors.RED)
        for violation in critical_violations[:3]:  # Show first 3
            print_colored(f"  {Path(violation.file_path).name}:{violation.line_number}", Colors.WHITE)
        ...
        print_colored("✅ BUILD CAN PROCEED: No critical security violations", Colors.GREEN)
...
def main() -> None:
    ...
    try:
        ...
        print_colored("✅ All security scanner examples completed successfully!", Colors.GREEN)
