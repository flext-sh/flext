#!/usr/bin/env python3
"""Restore modified __init__.py files across workspace git repos.

Equivalent to the legacy ``restore_init_files.sh`` helper. Finds every git
repository under the workspace, lists staged/unstaged ``__init__.py`` changes,
restores them, then verifies that the core packages still import cleanly.
"""
# /// flext-command
# verb = "clean"
...
def _changed_init_files(repo: Path) -> list[str]:
    ...
    for flag in ("--cached", None):
        ...
        if result.success:
            changed.update(line.strip() for line in result.value.splitlines() if line.strip())
...
def _validate_imports(workspace_root: Path) -> u.p.Result[bool]:
...
def main() -> int:
    ...
    workspace_root = Path(u.Cli.process_env().get("WORKSPACE_ROOT", os.getcwd())).resolve()
    if os.environ.get("APPLY", "N").upper() != "Y":
