...
for filename in os.listdir(reports_dir):
    if filename.endswith('.log'):
        ...
        with open(filepath, 'r') as f:
            content = f.read()
            match = re.search('ERROR: ' + re.escape(project) + '\\s+(\\d+)\\s+\\((.*?)\\)', content)
            if match:
                ...
                for item in details_str.split(', '):
                    key, val = item.split('=')
                    details[key] = int(val)
                results[project] = {'total': total, 'details': details, 'status': 'FAIL' if total > 0 else 'PASS'}
            elif 'Success: 1' in content or 'Success: 5' in content:
