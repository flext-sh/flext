...
TOTAL_COUNT=$((MODEL_REBUILD_COUNT + DICT_V1_COUNT + PARSE_OBJ_COUNT + PARSE_RAW_COUNT + JSON_V1_COUNT))
...
echo "Summary: model_rebuild=$MODEL_REBUILD_COUNT v1_dict=$DICT_V1_COUNT v1_parse_obj=$PARSE_OBJ_COUNT v1_parse_raw=$PARSE_RAW_COUNT v1_json=$JSON_V1_COUNT total=$TOTAL_COUNT"
...
if [[ "$MODE" == "strict" ]]; then
  ...
  BASE_TOTAL=$((BASE_REBUILD + BASE_DICT + BASE_PARSE_OBJ + BASE_PARSE_RAW + BASE_JSON))

  if [[ "$BASELINE_STRATEGY" == "per_group" ]]; then
    if [[ "$MODEL_REBUILD_COUNT" -gt "$BASE_REBUILD" || "$DICT_V1_COUNT" -gt "$BASE_DICT" || "$PARSE_OBJ_COUNT" -gt "$BASE_PARSE_OBJ" || "$PARSE_RAW_COUNT" -gt "$BASE_PARSE_RAW" || "$JSON_V1_COUNT" -gt "$BASE_JSON" ]]; then
      FAILED=1
    fi
  else
  ...
  echo "Baseline: model_rebuild=$BASE_REBUILD v1_dict=$BASE_DICT v1_parse_obj=$BASE_PARSE_OBJ v1_parse_raw=$BASE_PARSE_RAW v1_json=$BASE_JSON"
  echo "Delta: model_rebuild=$((MODEL_REBUILD_COUNT - BASE_REBUILD)) v1_dict=$((DICT_V1_COUNT - BASE_DICT)) v1_parse_obj=$((PARSE_OBJ_COUNT - BASE_PARSE_OBJ)) v1_parse_raw=$((PARSE_RAW_COUNT - BASE_PARSE_RAW)) v1_json=$((JSON_V1_COUNT - BASE_JSON)) total=$((TOTAL_COUNT - BASE_TOTAL))"
fi
