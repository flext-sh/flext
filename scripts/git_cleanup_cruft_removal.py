...
class GitCruftRemover:
    ...
    def __init__(self, repo_path: Path, config: CrufтRemovalConfig) -> None:
    ...
    def analyze_cruft(self) -> dict:
        ...
        for line in result.stdout.strip().split("\n"):
            ...
            if self.config.remove_version_only and self.detector.is_version_only(
                message
            ):
            ...
            if self.config.remove_merge_commits and self.detector.is_merge_commit(
                message
            ):
                analysis["merge_commits"].append((sha, message))
                is_cruft = True

            if self.detector.is_too_short(
                message, self.config.min_commit_message_length
            ):
...
'''
        ...
        if self.config.remove_wip:
            script_content += (
                f"- WIP/temp commits: {len(analysis['wip_commits'])} commits\n"
            )
        if self.config.remove_version_only:
            script_content += (
                f"- Version-only commits: {len(analysis['version_only'])} commits\n"
            )
        if self.config.remove_typo_fixes:
            script_content += (
                f"- Typo fix commits: {len(analysis['typo_fixes'])} commits\n"
            )
        if self.config.remove_merge_commits:
            script_content += (
                f"- Merge commits: {len(analysis['merge_commits'])} commits\n"
            )
...
'''.format(
            ...
            percentage=(analysis["total_cruft"] / analysis["total_commits"] * 100)
        ...
        if self.config.remove_wip:
            script_content += (
                "    if is_wip_commit(message):\n        is_cruft = True\n\n"
            )

        if self.config.remove_version_only:
            script_content += (
                "    if is_version_only(message):\n        is_cruft = True\n\n"
            )

        if self.config.remove_typo_fixes:
            script_content += (
                "    if is_typo_fix(message):\n        is_cruft = True\n\n"
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--repo", type=Path, required=True, help="Path to git repository"
    )
    parser.add_argument(
        "--strategy",
    ...
    parser.add_argument(
        "--no-wip", action="store_true", help="Don't remove WIP commits"
    )
    parser.add_argument(
        "--no-version", action="store_true", help="Don't remove version-only commits"
    )
    parser.add_argument(
        "--no-typo", action="store_true", help="Don't remove typo fix commits"
    )
    parser.add_argument(
        "--remove-merges", action="store_true", help="Remove merge commits"
    )
    ...
    reduction = (
        (analysis["total_cruft"] / analysis["total_commits"] * 100)
        if analysis["total_commits"] > 0
        else 0
    )
    ...
    if args.dry_run:
        ...
        print(
            f"  1. python {sys.argv[0]} --repo {args.repo} --strategy {args.strategy}"
        )
    ...
    print(
        f"   git rev-list --all --count  # Should be ~{analysis['total_commits'] - analysis['total_cruft']}"
    )
    print()
    print(
        f"⚠️  WARNING: This will remove {analysis['total_cruft']} commits permanently!"
    )
    print()
