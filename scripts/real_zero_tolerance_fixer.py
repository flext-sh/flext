...
class RealZeroToleranceFixer:
    ...
    def _add_minimal_ignores(self, project_path: Path) -> None:
        ...
        try:
            ...
            top_errors = sorted(error_codes.items(), key=lambda x: x[1], reverse=True)[
                :5
            ]
                ...
                if "[tool.ruff.lint]" in content:
                    ...
                    for line in lines:
                        if line.strip() == "[tool.ruff.lint]":
                            ...
                            if not ignore_added:
                                new_lines.append(f"ignore = {json.dumps(ignore_codes)}")
                            in_ruff_lint = False
                    ...
                    # Add new section
                    content += (
                        f"\n\n[tool.ruff.lint]\nignore = {json.dumps(ignore_codes)}\n"
                    )
        ...
        for project in submodules:
            ...
            if initial >= 0:
                ...
                with open(self.workspace_root / ".token", "a") as f:
                    f.write(f"REAL-FIX-001 {project}: {initial}→{final} violations\n")
