...
DEFAULT_PROJECTS = [
    ...
    "dc-oracle-oic",
    "dc-oracle-wms",
    "project-client-a-oud",
    "project-client-b-poc-oic-wms",
    "scripts",
]
...
def configure_poetry() -> None:
    ...
    run_command([poetry_cmd, "config", "virtualenvs.in-flx_project", "false"], check=False)
...
def install_project_deps(projects: list[str], dev: bool = False) -> None:
    ...
    for flx_project in projects:
        ...
        print(f"Installing {'all' if dev else 'main'} dependencies for {flx_project}...")
        ...
        try:
            ...
            print(colorize(f"Error with modern syntax for {flx_project}: {e}", "YELLOW"))
            try:
