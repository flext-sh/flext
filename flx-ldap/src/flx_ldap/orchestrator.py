...
class LDAPOrchestrator:
    ...
    def run_tap(
        ...
        try:
            ...
            logger.info(f"[green]✓[/green] Data extracted to {output_path}")
    ...
    def run_dbt(
        ...
        *,
        ...
        try:
            ...
            logger.info(f"[green]✓[/green] dbt {command} completed successfully")

            # Parse dbt output for summary
            if "Completed successfully" in process.stdout:
                logger.info(f"{process.stdout.split('Completed successfully')[0]}")

            return True

        except subprocess.CalledProcessError as e:
            logger.exception("dbt %s failed: %s", command, e.stderr)
            logger.info(f"[red]✗[/red] dbt {command} failed")
            return False
    ...
    def run_target(
        ...
        *,
        ...
        try:
            ...
            logger.info("[green]✓[/green] Data loaded successfully")
            return True
    ...
    def run_migration(
        ...
        *,
        ...
        logger.info("[bold]Starting LDAP migration...[/bold]")
    ...
    def _compare_extracts(self, source_path: Path, target_path: Path) -> None:
        ...
        try:
            ...
            for stream in sorted(all_streams):
                ...
                logger.info(f"  {stream}: {source} → {target} {status}")
    ...
    def validate_config(self) -> bool:
        ...
        if issues:
            logger.info("[bold red]Configuration issues found:[/bold red]")
            for issue in issues:
                logger.info(f"  • {issue}")
            return False

        logger.info("[green]✓[/green] Configuration is valid")
        return True
