...
planning capabilities extracted and enhanced from algar-oud-mig.
"""
...
# ZERO TOLERANCE: Use TYPE_CHECKING for optional dependencies
if TYPE_CHECKING:
    from pathlib import Path

    try:
        from ldap_core_shared.domain import (
            DomainModel,  # type: ignore[import-not-found]
        )
    except ImportError:
        DomainModel = object  # type: ignore[misc,assignment]

# Runtime import handling
try:
    from ldap_core_shared.domain import DomainModel  # type: ignore[import-not-found]

    LDAP_CORE_AVAILABLE = True
except ImportError:

    class DomainModel:  # type: ignore[no-redef]
        """Fallback DomainModel for when ldap_core_shared is not available."""
...
class SchemaAnalyzer:
    ...
    checking capabilities extracted from algar-oud-mig.
    """
    ...
    def analyze_ldap_schema(
        ...
        try:
            ...
            logger.exception("Failed to analyze schema: %s", e)
    ...
    def compare_schemas(
        ...
        compatibility_report = self._compare_schema_definitions(
            source_result.source_schema,
            target_result.source_schema,
        )
    ...
    def _extract_schema_from_catalog(self, catalog: dict[str, Any]) -> dict[str, Any]:
        ...
        object_classes_set: set[str] = set()
                ...
                if prop_name == "objectClass":
                    ...
                    for oc in enum_values:
                        if oc not in self.STANDARD_OBJECT_CLASSES:
                            object_classes_set.add(oc)

        # Convert set to list for JSON serialization
        schema_data["object_classes"] = list(object_classes_set)
    ...
    def extract_ldif_schema(self, ldif_file_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception("Error processing LDIF file: %s", e)
            schema_data["error"] = str(e)

        # Convert sets to lists for JSON serialization
        if isinstance(schema_data["attributes_found"], set):
            schema_data["attributes_found"] = list(schema_data["attributes_found"])
        if isinstance(schema_data["object_classes_found"], set):
            schema_data["object_classes_found"] = list(
                schema_data["object_classes_found"],
            )
        # Convert defaultdict to regular dict for JSON serialization
        entry_types = schema_data["entry_types"]
        attribute_usage = schema_data["attribute_usage"]
        schema_data["entry_types"] = dict(entry_types)
        schema_data["attribute_usage"] = dict(attribute_usage)
    ...
    def validate_schema_consistency(
        ...
        for attr_name, attr_def in custom_attrs.items():
            checks_performed: list[str] = validation_report["checks_performed"]
            checks_performed.append(f"validate_attribute_{attr_name}")

            # Check for valid OID
            oid = attr_def.get("oid")
            if not oid or not re.match(r"^\d+(\.\d+)*$", oid):
                issues: list[str] = validation_report["issues"]
                issues.append(f"Attribute '{attr_name}' has invalid OID: {oid}")
        ...
        if compatibility:
            for oc_def in compatibility.custom_object_classes:
                ...
                checks_performed.append(f"validate_objectclass_{oc_def.name}")
        ...
        if not custom_attrs and not (
            ...
            warnings.append("No custom schema elements detected")
