...
class TestFlxLDAPCompleteE2E:
    ...
    def test_transform_pipeline(
        ...
        assert table_exists_postgres(
            postgres_connection, "staging_ldap", "stg_org_units"
        )

        # Verify dimensional tables were created
        assert table_exists_postgres(postgres_connection, "analytics_ldap", "dim_users")
        assert table_exists_postgres(
            postgres_connection, "analytics_ldap", "dim_groups"
        )
        assert table_exists_postgres(
            postgres_connection, "analytics_ldap", "fact_memberships"
        )
    ...
    def test_error_recovery(
        ...
        with open(migration_config_file) as f:
            config = json.load(f)
    ...
    def test_custom_stream_migration(
        ...
        assert access_groups >= 2  # ldap-REDACTED_LDAP_BIND_PASSWORDs, vpn-users
    ...
    def test_client-a_oud_mig_compatibility(
        self,
        migration_config_file: Path,
        data_dir: Path,
    ) -> None:
        """Test compatibility with client-a-oud-mig migration patterns."""
        # Test specific patterns used by client-a-oud-mig

        # 1. Custom filters for specific object types
        with open(migration_config_file) as f:
            config = json.load(f)

        # Add client-a-specific custom streams
        ...
        client-a_config_file = data_dir / "client-a-config.json"
        with open(client-a_config_file, "w") as f:
            json.dump(config, f)

        # Test extraction with client-a patterns
        result = run_flx_ldap_command(
            ["extract", "--output", str(data_dir / "client-a-extract.jsonl")],
            client-a_config_file,
        )

        assert result.returncode == 0

        # Verify filtering worked (service accounts should be excluded from users)
        output_file = data_dir / "client-a-extract.jsonl"
        user_count = count_jsonl_records(output_file, "users")
