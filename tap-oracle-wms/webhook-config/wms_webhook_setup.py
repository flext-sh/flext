...
class WMSWebhookConfig:
        ...
        for entity in entities:
            try:
                ...
                if response.status_code == HTTP_OK:
                    ...
                    logger.info(f"✅ {entity}: Webhook support available")
                elif response.status_code == HTTP_NOT_FOUND:
                    supported_entities[entity] = False
                    logger.info(f"❌ {entity}: No webhook support")
                else:
                    supported_entities[entity] = False
                    logger.warning(
    ...
    def configure_entity_webhook(self, entity: str, webhook_url: str) -> bool:
        ...
        logger.info(f"Configuring webhook for entity: {entity}")
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                logger.info(f"✅ Webhook configured for {entity}")
                return True
            logger.error(
...
def main() -> None:
    ...
    logger.info(f"WMS URL: {base_url}")
    ...
    try:
        ...
        logger.info(f"✅ Supported: {supported_count}/{total_count} entities")
        ...
        if existing:
            logger.info(f"Found webhooks configured for: {', '.join(existing.keys())}")
        else:
