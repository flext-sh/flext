...
async def analyze_entity_relationships():
    ...
    for entity, rels in relationships.items():
        logger.info(f"\n{entity}:")
        for rel in rels:
            logger.info(f"  -> {rel['related_entity']} (via {rel['field']})")
...
async def performance_optimization_demo():
    ...
    for result in sorted(results, key=lambda x: x["records_per_second"], reverse=True):
        logger.info(f"  {result['strategy']}: {result['records_per_second']:.0f} rec/s")
    ...
    try:
        ...
        for stream, records in filtered_records.items():
            ...
            if records and stream == "inventory":
                ...
                logger.info(f"  Positive quantities: {positive_qty}")
                ...
                logger.info(f"  Recent orders (last 7 days): {recent_orders}")
                logger.info(f"  High priority orders: {high_priority}")
...
async def parallel_entity_extraction():
    ...
    for result in results:
        if result["success"]:
            ...
            logger.info(f"  {result['entity']}: FAILED - {result['error']}")

    logger.info(f"\nTotal: {total_records} records")
    logger.info(f"Throughput: {total_records / total_elapsed:.0f} records/sec")
