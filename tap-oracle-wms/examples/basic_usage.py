...
def discover_entities():
    ...
    try:
        ...
        logger.info(f"Discovered {len(catalog.streams)} entities:")
        for stream in catalog.streams:
            logger.info(f"  - {stream.tap_stream_id}")

            # Show primary keys and replication key
            metadata = stream.metadata[0].metadata
            if metadata.get("table-key-properties"):
                logger.info(f"    Primary keys: {metadata['table-key-properties']}")
            if metadata.get("replication-key"):
                logger.info(f"    Replication key: {metadata['replication-key']}")
...
def extract_sample_data() -> None:
    ...
    try:
        ...
        for stream, stream_records in records.items():
            logger.info(f"\n{stream}: {len(stream_records)} records")
            if stream_records:
                ...
                if len(sample) > MAX_RETRY_COUNT:
                    logger.info(f"    ... and {len(sample) - 5} more fields")
...
def incremental_sync_example() -> None:
    ...
    try:
        ...
        if new_state:
            ...
            logger.info(f"Saved new state: {new_state}")
