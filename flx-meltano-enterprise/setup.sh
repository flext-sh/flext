...
echo "from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.create_superuser('REDACTED_LDAP_BIND_PASSWORD', 'REDACTED_LDAP_BIND_PASSWORD@internal.invalid', 'REDACTED_LDAP_BIND_PASSWORD123') if not User.objects.filter(username='REDACTED_LDAP_BIND_PASSWORD').exists() else None" | cd src && poetry run python -m flx_web.manage shell || echo "⚠️  Superuser creation skipped"
cd ..
...
echo "5. Access Django REDACTED_LDAP_BIND_PASSWORD at http://localhost:8000/REDACTED_LDAP_BIND_PASSWORD (REDACTED_LDAP_BIND_PASSWORD/REDACTED_LDAP_BIND_PASSWORD123)"
echo ""
