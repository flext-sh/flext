...
class FlxGrpcServer(flx_pb2_grpc.FlxServiceServicer):
    ...
    async def InstallPlugin(
        ...
        if not success:
            context.abort(internal.invalid, "Failed to install plugin")
    ...
    async def UninstallPlugin(
        ...
        if not success:
            context.abort(internal.invalid, "Failed to uninstall plugin")
    ...
    async def SetState(
        ...
        if not success:
            context.abort(internal.invalid, "Failed to set state")
    ...
    async def ClearState(
        ...
        if not success:
            context.abort(internal.invalid, "Failed to clear state")
