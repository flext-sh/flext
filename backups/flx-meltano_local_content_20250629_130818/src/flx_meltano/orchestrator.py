...
class FlxMeltanoOrchestrator:
    ...
    async def _execute_pipeline_sync(self, flx_job: FlxJob, run_mode: RunMode = RunMode.FULL_RUN) -> dict[str, Any]:
        ...
        return {
            ...
            "duration_seconds": (
                (finished_at - flx_job.started_at).total_seconds() if finished_at and flx_job.started_at else None
            ),
        }

    async def _execute_pipeline_async(self, flx_job: FlxJob, run_mode: RunMode = RunMode.FULL_RUN) -> dict[str, Any]:
    ...
    async def _run_pipeline_task(self, project: Project, flx_job: FlxJob, run_mode: RunMode = RunMode.FULL_RUN) -> None:
    ...
    async def _execute_meltano_block(self, project: Project, flx_job: FlxJob, block: dict[str, Any]) -> dict[str, Any]:
    ...
    async def _execute_run_block(self, _project: Project, flx_job: FlxJob, commands: list[str]) -> dict[str, Any]:
        ...
        try:
            ...
            loader = commands[-1]
            transform = " ".join(commands[1:-1]) if len(commands) > min_commands else None
    ...
    async def _execute_invoke_block(self, project: Project, flx_job: FlxJob, commands: list[str]) -> dict[str, Any]:
