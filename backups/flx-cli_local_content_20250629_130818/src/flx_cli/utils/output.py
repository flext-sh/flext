"""Output formatting utilities for FLX CLI."""
...
def format_pipeline_list(console: Console, pipeline_list: Any) -> None:
    """Format and display pipeline list."""
    if not pipeline_list.pipelines:
        console.print("[yellow]No pipelines found[/yellow]")
        return

    table = Table(title=f"Pipelines (Page {pipeline_list.page} of {(pipeline_list.total + pipeline_list.page_size - 1) // pipeline_list.page_size})")
...
def format_pipeline(console: Console, pipeline: Any) -> None:
