...
from flx_plugin.core.base import Plugin, PluginInterface
...
class MetadataValidationRule(ValidationRule):
    ...
    def validate(self, plugin: Type[Plugin] | Plugin) -> tuple[bool, str | None]:
        ...
        if not version_pattern.match(metadata.version):
            return False, f"Invalid version format: {metadata.version} (expected semantic versioning)"
            
        # Validate plugin ID format
        id_pattern = re.compile(r"^[a-z0-9]+(-[a-z0-9]+)*$")
        if not id_pattern.match(metadata.id):
            return False, f"Invalid plugin ID format: {metadata.id} (expected lowercase with hyphens)"
...
class InterfaceValidationRule(ValidationRule):
    ...
    def __init__(self) -> None:
        ...
        super().__init__(
            ...
            "Validates plugin implements required interface"
        )
...
def validate_plugin_config(config: dict[str, Any], schema: dict[str, Any]) -> tuple[bool, list[str]]:
