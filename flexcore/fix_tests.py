...
def fix_pipeline_tests(content):
    ...
    content = re.sub(r'err := pipeline\.AddStep\(([^)]+)\)\n\s+require\.NoError\(t, err\)',
                     r'res := pipeline.AddStep(\1)\n\trequire.True(t, res.IsSuccess())', content)

    # Fix all err := pipeline.UpdateStep
    content = re.sub(r'err := pipeline\.UpdateStep\(([^)]+)\)\n\s+require\.NoError\(t, err\)',
                     r'res := pipeline.UpdateStep(\1)\n\trequire.True(t, res.IsSuccess())', content)

    # Fix all err := pipeline.RemoveStep
    content = re.sub(r'err := pipeline\.RemoveStep\(([^)]+)\)\n\s+require\.NoError\(t, err\)',
                     r'res := pipeline.RemoveStep(\1)\n\trequire.True(t, res.IsSuccess())', content)

    # Fix assert.Error calls
    content = re.sub(r'err := pipeline\.(AddStep|UpdateStep|RemoveStep)\(([^)]+)\)\n\s+assert\.Error\(t, err\)',
                     r'res := pipeline.\1(\2)\n\tassert.True(t, res.IsFailure())', content)

    # Fix assert.Contains for error messages
    content = re.sub(r'assert\.Contains\(t, err\.Error\(\), "([^"]+)"\)',
                     r'assert.Contains(t, res.Error().Error(), "\1")', content)
    ...
    # Fix Validate method
    content = re.sub(r'err := ([^.]+)\.Validate\(\)\n(\s+)if tt\.expectError \{',
                     r'res := \1.Validate()\n\2if tt.expectError {', content)

    content = re.sub(r'assert\.Error\(t, err\)\n(\s+)assert\.Contains\(t, err\.Error\(\), tt\.errorMsg\)',
                     r'assert.True(t, res.IsFailure())\n\1assert.Contains(t, res.Error().Error(), tt.errorMsg)', content)

    return re.sub(r'assert\.NoError\(t, err\)',
                     r'assert.True(t, res.IsSuccess())', content)
