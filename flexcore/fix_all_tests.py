...
def fix_pipeline_test_complete(content):
    ...
    content = re.sub(r'(entities\.PipelineStep\{[^}]+),\s*\}', r'\1,\n\t\tIsEnabled: true,\n\t}', content)
    ...
    for line in lines:
        ...
        if last_var == 'res' and 'err)' in line and 'assert.' in line:
            line = line.replace('err)', 'res)')
            line = line.replace('assert.Error(t, res)', 'assert.True(t, res.IsFailure())')
            line = line.replace('require.NoError(t, res)', 'require.True(t, res.IsSuccess())')
        ...
        if 'assert.Equal(t, "Updated description", pipeline.Steps[0].Description)' in line:
    ...
    content = re.sub(r'pipeline := entities\.NewPipeline\("benchmark", "description", "owner"\)\n\tpipeline\.AddStep',
                     'result := entities.NewPipeline("benchmark", "description", "owner")\n\trequire.True(b, result.IsSuccess())\n\tpipeline := result.Value()\n\tpipeline.AddStep', content)

    # Fix Order field in validation test
    return content.replace('{ID: "1", Name: "Step1", Type: "extractor", Order: 1}',
