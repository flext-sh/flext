...
- **AXIOMATIC** Compatibility aliases for types (`MyScalar = t.ScalarValue`, `ConfigDict = t.ConfigMap`, `AnyValue = t.GeneralValueType`, or any `X = t.Y` renaming) are TOTALLY FORBIDDEN. Reference `t.*` directly at every usage site. No indirection layers, no local re-exports, no renaming aliases for types. Not even "convenience" aliases.
...
#BT|- **AXIOMATIC** Python 3.12 `type X = ...` (PEP 695 TypeAliasType) creates annotation-only objects — they are TOTALLY FORBIDDEN as arguments to `isinstance()`, as base classes, or in any runtime type-checking context. Non-recursive type aliases that are used at runtime (isinstance, subclassing, TypeAdapter) MUST be declared as `X: TypeAlias = str | int | ...` (typing.TypeAlias), which produces a real UnionType. Recursive type aliases (Serializable, ContainerValue, JsonValue) MUST use `type X = ...` because they require forward references. Runtime type narrowing against `t.*` aliases MUST go through centralized `u.Guards.is_*` TypeGuard functions in `guards.py` — NEVER raw `isinstance(value, t.SomeAlias)` at call sites. The `guards.py` module is the SOLE source of truth for all runtime type narrowing tuples. Adding a new `isinstance` call against a `t.*` alias anywhere outside `guards.py` is a violation. The correct pattern: (1) non-recursive aliases → `X: TypeAlias = ...` in typings.py, (2) runtime narrowing → `u.Guards.is_primitive(x)` / `u.Guards.is_scalar(x)` in guards.py, (3) subclassing → use `collections.abc.Mapping[str, t.Container]` as the runtime base, not `t.ConfigurationMapping`.
...
- **CRITICAL: TypeAliasType isinstance trap (Python 3.12+)**: PEP 695 `type X = str | int` creates `TypeAliasType` — `isinstance(val, X)` FAILS at runtime. Non-recursive aliases MUST use `X: TypeAlias = str | int` (creates `UnionType`, isinstance-safe). Recursive aliases MUST use `type` statement but NEVER with `isinstance()` — use `TypeGuard` functions from `guards.py` instead. See `flext-core/CLAUDE.md` for complete rules and guards reference.
