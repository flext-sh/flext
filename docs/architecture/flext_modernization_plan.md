...
- **Verification status:** Targeted unit/integration tests were wired for each package (`flext-core/tests/unit/test_dispatcher.py`, `flext-ldap/tests/unit/test_domain_functional.py`, `flext-ldif/tests/integration/test_api.py`, `client-a-oud-mig/tests/unit/test_main.py`). Local execution surfaced missing shared test fixtures (`flext_tests` and `docker` dependencies), blocking runtime validation; suites should be re-run once those dependencies are restored.
