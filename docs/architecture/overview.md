...
Most `flext-*` projects are **LIBRARIES**, not services (30 of 33 projects):
...
```
MAIN WORKSPACE SERVICES (Go):
├── cmd/flext/              # Main FLEXT control service
├── cmd/flext-server/       # FLEXT server service
├── cmd/flext-cli/          # FLEXT CLI service
└── cmd/flext-demo/         # Demo service

FLEXCORE RUNTIME SERVICES (Go):
├── flexcore/cmd/flexcore/  # Core runtime service
├── flexcore/cmd/server/    # FlexCore server
└── flexcore/cmd/e2e-test/  # End-to-end testing service

CLIENT PROJECT SERVICES (Python):
├── client-a-oud-mig/          # client-a OUD migration service
└── client-b-meltano-native/ # client-b native service
```
