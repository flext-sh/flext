...
This directory keeps one-page summaries that mirror the authoritative `README.md` + `AGENTS.md` guidance inside each project. Think of each brief as the "project status" card that points readers back to the real implementation, the quality gate reports, and the release information stored in `reports/`.
...
- **[FLEXT API](./flext-api/README.md)** – version 0.9.9, fully covered HTTP client + FastAPI integration, production-ready FastAPI app factory,
  ...
  `make validate`; documentation surfaces live inside `docs/` and the `README`/`AGENTS.md` files in `flext-api/`.
...
- **[FLEXT DB Oracle](./flext-db-oracle.md)** – version 0.9.9, Oracle connectivity + schema introspection built on SQLAlchemy 2 + python-oracledb,
  ...
  zero tolerance pattern rules live in the project `README.md` + `AGENTS.md` docs.
- **[FLEXT DBT LDAP](./flext-dbt-ldap.md)** – version 1.0.0 release prep, dbt Core transformation suite for LDAP/AD with staging → intermediate →
  ...
  `make validate`. Documentation lives inside `flext-dbt-ldap/docs/` and the `README.md` + `AGENTS.md` files referenced below.
- **[FLEXT dbt LDIF](./flext-dbt-ldif.md)** – version 1.0.0 release prep, dbt project for LDAP/LDIF analytics with programmatic model generation,
  ...
  quality gates align with the `README.md` + `AGENTS.md` docs.
- **[FLEXT dbt Oracle](./flext-dbt-oracle.md)** – version 1.0.0 release prep, dbt Oracle integration with flext-db-oracle connectivity,
  ...
  `dbt docs serve`. Documentation links point to the project `README.md` + `AGENTS.md` and `docs/` tree.
...
setup`, `make validate`, run dbt/deploy commands; docs live inside `flext-meltano/docs/` and the AGENTS file.
- **[FLEXT Observability](./flext-observability.md)** – version 0.9.0, observability monitoring/tracing foundation with completed architecture,
  ...
  entity/services surface remains ready. Quick start: `poetry install`, `make setup`, `make validate` (blocked), import `flext_create_metric`/`flext_monitor_function`. Documentation lives inside the project `README.md` + `AGENTS.md` and `docs/` tree.
...
check`, `make validate` (imports currently blocked), instantiate `FlextQualityService`, and run the direct analyzer import; docs link to `README.md` + `AGENTS.md`
...
validate`, and `tap-ldap --discover/run`; docs live inside the project `README.md` + `AGENTS.md` and `docs/` tree.
- **[FLEXT Tap LDIF](./flext-tap-ldif.md)** – version 1.0.0 release prep, Singer tap for LDIF extraction with Clean Architecture, 90%+ coverage, and
  ...
  commands; docs reference the project `README.md` + `AGENTS.md` and `docs/` folder.
- **[FLEXT Tap Oracle](./flext-tap-oracle.md)** – version 1.0.0 release prep, Oracle Singer tap built on flext-db-oracle/flext-meltano, 90% coverage,
  ...
  the project `README.md` + `AGENTS.md` and `docs/` tree.
- **[FLEXT Tap Oracle OIC](./flext-tap-oracle-oic.md)** – version 1.0.0 release prep, Oracle Integration Cloud Singer tap with OAuth2/IDCS auth, 12+
  ...
  commands, and uses the tap CLI; docs live inside the project `README.md` + `AGENTS.md` and the `docs/` folder.
- **[FLEXT Tap Oracle WMS](./flext-tap-oracle-wms.md)** – version 1.0.0 release prep, Oracle Warehouse Management System Singer tap with 10 working
  ...
  commands, and references the TODO/refactor docs; docs live under the project `README.md` + `AGENTS.md` and `docs/` tree.
...
- **[FLEXT Target Oracle](./flext-target-oracle.md)** – version 0.9.9 (1.0.0 prep), Singer target for Oracle with strong documentation but blocked
  ...
  `FlextOracleTargetSettings`; docs link to `README.md` + `AGENTS.md` and the reports folder.
...
1. **Mirror the repository's narrative** – copy the official version, status, test counts, coverage, and quality gate (Ruff, Pyrefly, pytest, Bandit, etc.) from the project `README.md` + `AGENTS.md` so the brief never claims a phantom artifact.
