...
- **Integration points**: `flext-core` (FlextResult, container, logger, short aliases), `flext-ldif` (entry models, quirks, adapters), `flext-auth` (LDAP authentication provider), `flext-meltano`/Singer taps for data export, `client-a-oud-mig` for Oracle migrations.
...
- Related projects: `flext-core`, `flext-ldif`, `flext-auth`, `flext-meltano`, `client-a-oud-mig`
