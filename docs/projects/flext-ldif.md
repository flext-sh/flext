...
FLEXT LDIF is the RFC 2849/4512-compliant LDIF processor for the FLEXT platform. It combines a strict parser, a server registry for server-specific rules, and migration pipelines that can target Oracle (OID/OUD), OpenLDAP, Active Directory, and any RFC-compliant LDAP server.
...
Use `ldif.migrate(...)` for server-to-server conversions, `parse_with_auto_detection` to let the registry choose the servers, and the `FlextLdifModels` helpers for filtering and categorizing entries.

## Key capabilities

- **RFC-first parsing**: all processing flows through RFC 2849/4512 parsers before servers are applied.
- **Server registry**: auto-discovers Oracle, OpenLDAP, AD, 389DS, and custom servers and applies them via priority-based dispatch.
...
```
Input LDIF → RFC parser → Server adapters → Target writer/migration pipeline
```
...
- **Extensible servers**: `servers.registry`, `servers.server-specific`, `servers.conversion_matrix`, `servers/dn_case_registry`.
- **RFC compliance layer**: `rfc/` modules with parser/writer/schema helpers.

## Usage highlights

- **Auto-detect servers**: `parse_with_auto_detection` returns metadata that identifies the detected server server.
- **Server migrations**: `ldif.migrate(input_dir, output_dir, from_server, to_server)` pipelines entries through RFC + servers and writes structured output.
...
- [AGENTS guidance](../../flext-ldif/AGENTS.md)
