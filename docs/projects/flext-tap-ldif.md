...
- **Core components**: `FlextTapLdif` (Singer tap), `LDIFEntriesStream` (Singer stream), `TapLdifSettings` (Pydantic settings), `FlextLdifProcessorWrapper`, `exception` hierarchy, watchers, and CLI/hot reload helpers.
...
- **Configuration**: rich settings options (file/directory/batch/perf settings, filters, encoding, strict parsing) defined in docs and enforced through typed Pydantic models.
