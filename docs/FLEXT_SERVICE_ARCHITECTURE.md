...
**FLEXT é um ÚNICO SERVIÇO** que roda no namespace `flext` na workspace, implementando **Clean Architecture + Domain Driven Design (DDD)** com orquestração completa via **Plugin System + Dependency Injection**.

**CONCEITO CRÍTICO**: Os projetos `flext-core`, `flext-api`, `flext-auth`, `flext-cli`, `flext-grpc`, `flext-plugin`, `flext-web` **SÃO SIMPLESMENTE BIBLIOTECAS** que fornecem funcionalidades específicas para o serviço principal.

**IMPLEMENTAÇÃO REAL**: O serviço principal está implementado em Go em `/home/marlonsc/flext/cmd/flext/main.go` com arquitetura completa de bounded contexts e DI container.

---

## 🏛️ ARQUITETURA DO SERVIÇO FLEXT

### Estrutura Real do Serviço Principal

```
/home/marlonsc/flext/
├── cmd/flext/                      # 🚀 SERVIÇO PRINCIPAL (Go)
│   └── main.go                     # Entry point: Bootstrap + Container + Server
├── internal/                       # 🔧 IMPLEMENTAÇÃO INTERNA (Clean Architecture)
│   ├── bounded_contexts/           # 🎯 CONTEXTOS LIMITADOS (DDD)
│   │   ├── pipeline/              # Pipeline Domain Context
...
│   │   ├── meltano/               # Meltano Integration Context
│   │   ├── singer/                # Singer Protocol Context
...
│   ├── shared_kernel/             # 🔧 KERNEL COMPARTILHADO
│   │   ├── application/           # Bootstrap & App Patterns
│   │   ├── domain/                # Base Domain Types
│   │   └── infrastructure/        # Shared Infrastructure
│   └── usecases/                  # 📋 USE CASES POR DOMÍNIO
│       ├── pipeline/              # Pipeline Use Cases
│       ├── plugin/                # Plugin Use Cases
│       └── meltano/               # Meltano Use Cases
├── flext-*/                        # 📚 BIBLIOTECAS PYTHON (Importadas via gRPC/HTTP)
...
## 🔧 SISTEMA REAL DE DEPENDENCY INJECTION + BOUNDED CONTEXTS

### Arquitetura REAL: FLEXCORE Container + FLEXT Service + Bibliotecas
