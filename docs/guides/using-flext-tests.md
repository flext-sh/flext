...
Workspace-scoped checks must compute their default project set from declared
workspace metadata: `.gitmodules`, `tool.flext.workspace.members`, and
`tool.uv.workspace.members`. Do not hardcode external project names or infer
projects from dependency declarations. Pass an explicit project list only when a
command intentionally narrows scope.

Root pre-commit config is generated by `flext-infra` sync and should stay
tracked. Hooks must route through `uv run --all-packages python -m flext_infra`
for boundary, LOC cap, and manual-command validation instead of duplicating
shell or Python logic beside the Make surface.
