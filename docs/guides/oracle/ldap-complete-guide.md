...
export OID_PORT=389
export OID_ADMIN_USER=cn=orcladmin
export OID_ADMIN_PASSWORD=oid_admin_password
...
export OUD_ADMIN_PASSWORD=oud_admin_password
...
python -m oud_automation.cli schema export \
    ...
    --host $OID_HOST \
    --port $OID_PORT \
    --admin-user "$OID_ADMIN_USER" \
    --admin-password "$OID_ADMIN_PASSWORD" \
...
python -m oud_automation.cli schema import \
    ...
    --admin-user "$OUD_ADMIN_USER" \
    --admin-password "$OUD_ADMIN_PASSWORD" \
...
config = Configuration({
    ...
    'admin_password': os.getenv('OUD_ADMIN_PASSWORD'),
