...
| `test` | `all` | Always `--testmon`; local coverage unless `CI=Y`; `WHAT=cache-status|cache-clear|cache-checkpoint`; optional `FILE=` / `MATCH=` | `make test PROJECT=flext-infra` / `make test CI=Y` |
