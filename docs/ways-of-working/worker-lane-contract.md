...
- Local skills: [`flext-law`](../../.agents/skills/flext-law/SKILL.md)
...
- Nothing reaches `0.12.0-dev` except through the lane's own reviewed PR: one bead ->
  ...
  (`~/.agents/skills/tool/pr-sheriff/scripts/pr_triage.py gate <owner/repo> <pr> --base`
  `0.12.0-dev --head <oid>`) -> independent review or operator-authorized administrative
  merge -> merge commit -> post-merge runtime proof -> bead evidence -> branch cleanup.
