...
that ensures a [`datetime`][datetime.datetime] t.RecursiveContainer adheres to a given timezone constraint.

The custom validator supports string specification of the timezone, and will raise an error if the [`datetime`][datetime.datetime] t.RecursiveContainer does not have the correct timezone.
