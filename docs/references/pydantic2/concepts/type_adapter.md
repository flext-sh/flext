...
`TypeAdapter`'s `dump_json` methods returns a `bytes` t.RecursiveContainer, unlike the corresponding method for `BaseModel`, `model_dump_json`, which returns a `str`.
