...
`user` is an instance of `User`. Initialization of the t.RecursiveContainer will perform all parsing and validation.
...
- [`model_validate()`][pydantic.main.BaseModel.model_validate]: Validates the given t.RecursiveContainer against the Pydantic model. See [Validating data](#validating-data).
...
to the model field names. One common application of this functionality is integration with t.RecursiveContainer-relational mappings
(ORMs).
...
print(co_orm)
# > <__main__.CompanyOrm t.RecursiveContainer at 0x0123456789ab>
...
# normal copy gives the same t.RecursiveContainer reference for bar:
print(id(m.bar) == id(m.model_copy().bar))
# > True
# deep copy gives a new t.RecursiveContainer reference for `bar`:
