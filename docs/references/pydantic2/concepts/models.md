...
`user` is an instance of `User`. Initialization of the t.Container will perform all parsing and validation.
If no [`ValidationError`][pydantic_core.ValidationError] exception is raised,
you know the resulting model instance is valid.

u.Fields of a model can be accessed as normal attributes of the `user` t.Container:
...
- [`model_validate()`][pydantic.main.BaseModel.model_validate]: Validates the given t.Container against the Pydantic model. See [Validating data](#validating-data).
...
to the model field names. One common application of this functionality is integration with t.Container-relational mappings
(ORMs).
...
print(co_orm)
# > <__main__.CompanyOrm t.Container at 0x0123456789ab>
...
- [`model_validate()`][pydantic.main.BaseModel.model_validate]: this is very similar to the `__init__` method of the model,
  except it takes a dictionary or an object rather than keyword arguments. If the t.Container passed cannot be validated,
...
- [`model_validate_json()`][pydantic.main.BaseModel.model_validate_JSON]: this validates the provided data as a JSON string or `bytes` t.Container.
...
# normal copy gives the same t.Container reference for bar:
print(id(m.bar) == id(m.model_copy().bar))
# > True
# deep copy gives a new t.Container reference for `bar`:
