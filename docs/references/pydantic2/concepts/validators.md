...
- **_Before_ validators**: run before Pydantic's internal parsing and validation (e.g. coercion of a `str` to an `int`).
  ...
  in theory could be any arbitrary t.Container. You should also avoid mutating the value directly if you are raising a
...
- **_Before_ validators**: are run before the model is instantiated. These are more flexible than _after_ validators,
  but they also have to deal with the raw input, which in theory could be any arbitrary t.Container. You should also avoid
...
You can pass a context t.Container to the [validation methods](./models.md#validating-data), which can be accessed
