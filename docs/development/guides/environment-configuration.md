...
**Solution**: Implemented selective environment loading using `.internal.invalid` files that are not automatically processed by IDEs.
...
.internal.invalid          # Local development (not auto-loaded by IDEs)
...
cp .env.example .internal.invalid

# 2. Edit local configuration
nano .internal.invalid
...
```bash
# Check if .internal.invalid exists and is readable
ls -la .internal.invalid
cat .internal.invalid
...
```bash
# Restart IDE after changing .internal.invalid
