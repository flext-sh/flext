...
| S2b | Spike accepted by the three checkers without suppression; if so, typed `resolve(Port)`, `compose` failures on missing or invalid ports, two services sharing one adapter; otherwise evidence and D3 | Same | Core suite |
...
2. the root composes it by constructor (pure DI) and, if S2b passed, by `compose`;
3. `flext-cli` `service_routes(Service, provide=…)` registers the routes on an app
   built by `create_app_with_common_params`;
4. through `execute_app` and `finalize_result`: a valid command exits 0 with the rendered
   result, an invalid one exits non-zero with the validation cause, and `--help` works
   without adapters;
