...
`~/wip-hier.sh` vira shim de 3 linhas: `exec /home/marlonsc/wip-hier/bin/wip-hier "$@"` — docs, handoffs e o plano rope-modernize continuam válidos sem edição.
...
7. Atualizar Kilo allowlist (permissões): adicionar `/home/marlonsc/wip-hier/bin/wip-hier *` (manter `/home/marlonsc/wip-hier.sh *` se existir).
...
3. **Paridade dry-run na frota real:** a partir de `/home/marlonsc/flext-worktrees/rope-modernize`, rodar back-to-back `bash ~/wip-hier/.baseline/wip-hier-v1.sh` e `~/wip-hier/bin/wip-hier`; `diff` das saídas normalizando linhas voláteis (`time:`, `log:`, caminhos com pid/ROWFILE). ZERO diferença no restante.
