...
**D4 — Reds estruturais zerados** (ator): namespace 83, loc-cap 5 (config.py 3.695 LOC, conform.py 2.986 LOC — _splits obrigatórios pela SUPREME LAW de 1k LOC_), duplication 57 (`flext-uuhc4`), mypy 38, tier-whitelist 712 violações (`flext-y3qpq.3`). Gatilho de absorção: a cada pouso do ator, revalidar gates antes de qualquer nova lane.
...
_Plano vivente — atualizar a cada material-step. Próxima revisão: após `flext-2h0un` ou pouso do ator, o que vier primeiro._
...
_Se (a) aprovado: abrir lane, aplicar o ciclo §11.1 nas ondas P1->P3, reportar por beads + este plano vivente._
