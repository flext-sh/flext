...
class FlxSyntaxFixer:
    ...
    def find_syntax_error_files(self) -> list[Path]:
        ...
        try:
            result = subprocess.run(
                ["find", str(self.flx_root), "-name", "*.py", "-not", "-path", "*/.venv/*"],
                capture_output=True,
                text=True,
                check=True
            )
    ...
    def fix_import_syntax_errors(self, file_path: Path) -> bool:
        ...
        try:
            ...
            while i < len(lines):
                ...
                if line == "import sys" and i > 0:
                    ...
                    if (not prev_line.startswith('"""') and
                        not prev_line.startswith('from __future__') and
                        not prev_line.startswith('import ') and
                        not prev_line.startswith('from ') and
                        not next_line.startswith('from ') and
                        not next_line.startswith('import ')):
                        i += 1
                        continue

                # Fix duplicate import lines
                if (line.startswith('from typing import') and
                    any(prev_line.startswith('from typing import') for prev_line in fixed_lines[-3:])):
                    i += 1
                    continue
            ...
            content = re.sub(r'from typing import ([^,\n]+(?:, [^,\n]+)*)(, [^,\n]+)*\nfrom typing import.*',
                           r'from typing import \1', content, flags=re.MULTILINE)

            # Clean up any remaining syntax issues
            # Remove lines that are just standalone keywords
            content = re.sub(r'^\s*from __future__ import annotations\s*$\n^\s*import sys\s*$\n^\s*from typing.*$\n^\s*from typing.*$\n',
    ...
    def fix_all_syntax_errors(self) -> None:
        ...
        with open(self.flx_root.parent / ".token", "a") as f:
            f.write(f"FLX-SYNTAX-FIX-002 PROGRESS: Fixed {len(self.fixed_files)} syntax errors\n")
