...
```bash
# Old: cd algar-oud-mig && python analyze_hierarchy_errors.py
./flx migration run algar-oud --dry-run

# Old: cd algar-oud-mig && python complete_groups_migration.py
./flx migration run algar-oud

# Old: cd gruponos-meltano-native && python production_meltano_test.py
./flx migration run gruponos
```
