...
class LDAPConnectionPool:
    ...
    def __init__(
        self, config: LDAPServerConfig, pool_size: int = 5, max_retries: int = 3
    ):
...
class LDAPOperationHelper:
    ...
    def __init__(self, connection_pool: LDAPConnectionPool):
    ...
    async def add_entry(self, entry: LDAPEntry) -> bool:
        ...
        try:
            async with self.pool.get_connection() as conn:
                ...
                if not success:
                    self.logger.error(f"Add operation failed: {conn.result}")
            ...
            self.logger.error(f"Add operation failed: {e}")
            return False
    ...
    async def modify_entry(self, dn: str, changes: dict[str, Any]) -> bool:
        ...
        try:
            async with self.pool.get_connection() as conn:
                ...
                if not success:
                    self.logger.error(f"Modify operation failed: {conn.result}")
    ...
    async def delete_entry(self, dn: str) -> bool:
        ...
        try:
            async with self.pool.get_connection() as conn:
                ...
                if not success:
                    self.logger.error(f"Delete operation failed: {conn.result}")
