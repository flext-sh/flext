...
func (m *UnifiedAuthMiddleware) RequirePermission(permissions ...string) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if !m.hasAnyPermission(userContext.Permissions, permissions) && !m.hasAnyRole(userContext.Roles, []string{"admin"}) {
...
// RequireAdmin is a route-level helper for admin access
