...
provides:
  - StrEnum + strict Pydantic coercion via m.BeforeValidator pattern
...
tech-stack:
  added: []
  patterns: [m.BeforeValidator for StrEnum coercion on strict Pydantic models]
...
key-decisions:
  - "m.BeforeValidator lambda pattern for StrEnum coercion (fewer touch points than fixing all call sites)"
  - "Applied m.BeforeValidator to all 5 StrEnum fields across 3 model classes (CreateParams, ReadParams, CreateKwargsParams, BatchParams)"

patterns-established:
  - "m.BeforeValidator coercion: use m.BeforeValidator(lambda v: EnumType(v) if isinstance(v, str) else v) on all StrEnum fields in strict Pydantic models"
...
**m.BeforeValidator pattern fixes StrEnum+strict Pydantic coercion across 5 fields, deprecation framework stubbed per FROZEN policy, UserDict/UserString confirmed absent**
...
- Fixed StrEnum coercion on 5 fields across 4 strict Pydantic model classes using m.BeforeValidator
...
- `flext-tests/src/flext_tests/models.py` - Added m.BeforeValidator to 5 StrEnum fields, enum defaults
...
- m.BeforeValidator lambda pattern chosen over fixing all call sites (fewer touch points)
...
- **Fix:** Applied same m.BeforeValidator + enum default pattern to all 5 fields
