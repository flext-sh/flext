...
- D-04: Fix at call sites first; if too many, add `BeforeValidator`. Do NOT relax `strict=True`
...
- Whether to use BeforeValidator or fix call sites for StrEnum coercion
...
|---------|-------------|-------------|-----|
| StrEnum coercion | Manual str-to-enum conversion at every call site | `BeforeValidator` on Pydantic field | Single fix point, validated by Pydantic |
...
**How to avoid:** Use `BeforeValidator` to coerce strings to enum instances before strict validation runs. The validator runs in the "before" phase, before Pydantic's strict type check.
