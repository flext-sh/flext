...
**Domain:** DIP enforcement, Pydantic v2 u.Field() canonicalization, ABC-to-Protocol conversion, PEP 695 type aliases, import normalization
**Confidence:** HIGH

## Summary

Phase 2 applies Dependency Inversion Principle via protocols across 33 projects, canonicalizes Pydantic v2 u.Field() patterns, converts ABCs to Protocols where safe, caches TypeAdapter instances, fixes mutable defaults, and normalizes import patterns. The sisyphus plans provide exhaustive audits with exact file locations and counts. Phase 1 guarantees a clean type system baseline (0 pyrefly/pyright errors).

The work is requirement-clustered into 4 waves: (0) issubclass prerequisite fixes, (1) Protocol DIP enforcement, (2) u.Field() canonicalization + TypeAdapter + mutable defaults, (3) PEP 695 stragglers + import normalization. All changes are mechanical/structural with zero behavioral modifications.
...
- **D-05:** All ~1,551 `u.Field(...)` usages are in scope, **including tests**. `u.PrivateAttr()` (94 usages) is explicitly **excluded** -- it is not a u.Field pattern.
- **D-06:** Migration pattern: `x: T = u.Field(...)` -> `x: Annotated[T, u.Field(...)]`. For optional fields: `x: T | None = u.Field(...)` -> `x: Annotated[T | None, u.Field(...)]` (NOT `Annotated[T, u.Field(...)] | None`).
...
- The ast-grep rule design for u.Field() migration and protocol substitution is at Claude's discretion, informed by the sisyphus plan patterns.
...
| ARCH-03 | All ~1,551 `u.Field(...)` usages migrated to `Annotated[X, u.Field(...)]` | Sisyphus plan has per-project counts. ast-grep pattern: `$NAME: $TYPE = u.Field($$$)` -> `$NAME: Annotated[$TYPE, u.Field($$$)]`. u.PrivateAttr excluded. |
...
|------|---------|--------------|
| ast-grep (`sg`) | Mechanical code transformations (u.Field migration, protocol substitution) | Project standard per CLAUDE.md. Proven in Phase 1. |
...
### Pattern 2: u.Field() -> Annotated Migration (ARCH-03)

**What:** Canonical Pydantic v2 u.Field pattern.

```python
# BEFORE
name: str = u.Field(default="", description="Name")
items: t.StrSequence = u.Field(default_factory=list)
settings: Settings | None = u.Field(default=None)

# AFTER
name: Annotated[str, u.Field(default="", description="Name")]
items: Annotated[t.StrSequence, u.Field(default_factory=list)]
settings: Annotated[Settings | None, u.Field(default=None)]
```

**Critical:** `| None` goes INSIDE `Annotated[T | None, u.Field()]`, NOT outside as `Annotated[T, u.Field()] | None` (different Pydantic semantics).
...
- **Annotated[T, u.Field()] | None:** Wrong Pydantic semantics. Always `Annotated[T | None, u.Field()]`.
...
|---------|-------------|-------------|-----|
| u.Field migration regex | Custom regex/sed | ast-grep pattern matching | ast-grep understands AST structure, regex breaks on multiline u.Field() |
...
### Pitfall 1: Annotated Semantics for Optional u.Fields

**What goes wrong:** Using `Annotated[T, u.Field()] | None` instead of `Annotated[T | None, u.Field()]`
...
### Pitfall 4: u.Field() in Multiline Declarations

**What goes wrong:** ast-grep pattern fails on multiline u.Field() with keyword args
**Why it happens:** Simple pattern `$NAME: $TYPE = u.Field($$$)` may not match multiline
**How to avoid:** Test ast-grep patterns with dryRun=true first. Use multiline-aware patterns.
**Warning signs:** grep shows remaining non-Annotated u.Field() after migration
...
### ast-grep: u.Field() -> Annotated Migration

```yaml
# sg rule for simple u.Field migration
id: field-to-annotated
language: python
rule:
  pattern: "$NAME: $TYPE = u.Field($$$ARGS)"
fix: "$NAME: Annotated[$TYPE, u.Field($$$ARGS)]"
```
...
| ARCH-03 | u.Field() in Annotated form | grep audit | `grep -rn "u.Field(" --include="*.py" */src/ \| grep -v "Annotated\[" \| grep -v u.PrivateAttr` -> 0 | N/A (grep) |
...
| u.Field(default=[]) | 2 in flext-dbt-oracle | Others may have been fixed; verify at execution |
