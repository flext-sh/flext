...
### 3. Annotated u.Field Form

expected: Zero `$NAME: $TYPE = u.Field(` pattern remaining in src/ (all migrated to `Annotated[T, u.Field(...)]`). Zero `u.Field(default=[])` or `u.Field(default={})` mutable defaults.
result: pass
