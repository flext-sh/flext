...
**Policy gate** (`make pol`) already exists and enforces zero `Any`, zero `object`, zero `# type: ignore`. This gate is the evidence artifact for TYPE-02/TYPE-03 acceptance.
...
- **Mechanical `object` → `object` swaps**: `object` is also FORBIDDEN. Replace with the most specific type available from the `t.*` hierarchy.
...
| `make pol` | Repo-wide | Zero `Any`, zero `object`, zero `# type: ignore` |
