...
**Policy gate** (`make pol`) already exists and enforces zero `Any`, zero `t.RecursiveContainer`, zero `# type: ignore`. This gate is the evidence artifact for TYPE-02/TYPE-03 acceptance.
...
| policy gate | `make pol` | Zero Any/t.RecursiveContainer/type: ignore | TYPE-02, TYPE-03 |
...
- **Mechanical `object` → `t.RecursiveContainer` swaps**: `t.RecursiveContainer` is also FORBIDDEN. Replace with the most specific type available from the `t.*` hierarchy.
...
| `make pol` | Repo-wide | Zero `Any`, zero `t.RecursiveContainer`, zero `# type: ignore` |
