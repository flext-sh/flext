...
- `.sisyphus/plans/bare-object-elimination.md` — Bare `object`/`dict`/`list` annotation strategy: new type aliases to create (`t.RecursiveValue`, `t.LoggerFactory`, etc.), ~350 annotation fixes, 7-wave execution plan.
- `.sisyphus/plans/pyright-zero-errors.md` — Pyright error distribution (~2,098 issues), root causes (`_operation_stats` u.PrivateAttr pattern = 121+ cascade, `_GuardInput` too narrow), wave plan.
