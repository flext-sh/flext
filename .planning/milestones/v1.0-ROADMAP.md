...
**Success Criteria** (what must be TRUE):
  ...
  1. All public API signatures reference `p.*` protocol types (`p.Context`, `p.DI`, `p.Settings`, `p.StructlogLogger`) — no concrete class leakage
  2. `c,m,t,u,p` are never imported from `flext_core` directly in `tests/`, `examples/`, or `scripts/` — always from local namespace root
  3. All ~1,551 `u.Field(...)` usages use `Annotated[X, u.Field(...)]` canonical Pydantic v2 form
  4. 6 pure ABCs converted to `@runtime_checkable` Protocol; 8 template ABCs have Protocol interface extracted
  5. `TypeAdapter()` instantiations cached as `ClassVar`/module constants; mutable `u.Field(default=[])` replaced with `default_factory=list`; type aliases use PEP 695 `type X = ...`
**Plans**: 5 plans
...
- [x] 02-03-PLAN.md — u.Field()->Annotated migration + mutable defaults fix (all 33 projects)
