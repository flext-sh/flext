...
- Private attributes: prefix with underscore (`self._state`), use Pydantic `u.PrivateAttr()` for BaseModel fields
...
- Use `u.Field()` with constraints, descriptions, and defaults
- Use `model_config = ConfigDict(...)` for serialization/validation settings
- Use `u.PrivateAttr()` for internal mutable state
- Use `u.field_validator` or `u.model_validator` for custom logic
