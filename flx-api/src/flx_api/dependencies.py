...
def get_current_user(
    ...
    if config.is_production and ("dev-secret" in jwt_secret.lower() or "change" in jwt_secret.lower()):
...
def get_execution_service(
    service: ExecutionService = Provide[ApplicationContainer.services.execution_service],
) -> ExecutionService:
...
@inject
def get_plugin_service(service: PluginService = Provide[ApplicationContainer.services.plugin_service]) -> PluginService:
    """Get plugin service with dependency injection."""
    return service


@inject
def get_event_bus(event_bus: HybridEventBus = Provide[ApplicationContainer.eventing.event_bus]) -> HybridEventBus:
    """Get event bus for domain event publishing."""
    return event_bus


@inject
def get_unit_of_work(uow: UnitOfWork = Provide[ApplicationContainer.database.unit_of_work]) -> UnitOfWork:
