...
WORKSPACE_BOOT_LOCK ?= /tmp/flext-workspace-boot.lock
export UV_CACHE_DIR
...
# Workspace verb hook seam: workspace_custom.mk may define pre-<verb>,
# post-<verb>, pre-<verb>-<what>, and post-<verb>-<what> targets to append work
# at the start or end of any root verb, for all or some WHATs. Undefined hooks
# are no-ops (make -q returns 2 when a target is absent). $(1)=phase (pre|post),
# $(2)=verb, $(3)=optional WHAT.
define _run_workspace_hooks
...
$(WORKSPACE_INFRA_WORKSPACE) sync --workspace "$(CURDIR)" --apply >/dev/null || exit 1
endef
...
define PREFLIGHT_CHECK
	...
	if [ -z "$(PYTHON_CMD)" ] || ! command -v $(PYTHON_CMD) >/dev/null 2>&1; then \
		...
		py_minor=$$($(PYTHON_CMD) -c 'import sys; u.Cli.print(sys.version_info.minor)' 2>/dev/null || echo 0); \
...
define ENSURE_CLEAN_SUBMODULE_WORKTREES
dirty_submodules=""; \
for submodule_path in $$submodule_paths; do \
	if [ -e "$$submodule_path/.git" ] && [ -n "$$(git -C "$$submodule_path" status --porcelain --untracked-files=all)" ]; then \
		dirty_submodules="$$dirty_submodules $$submodule_path"; \
	fi; \
done; \
if [ -n "$$dirty_submodules" ]; then \
	echo "ERROR: boot refused to update dirty submodule worktrees:"; \
	for submodule_path in $$dirty_submodules; do echo " - $$submodule_path"; done; \
	echo "Commit or otherwise preserve those changes before rerunning make boot."; \
	exit 1; \
fi
endef

.PHONY: help boot _boot_default _boot_venv _boot_submodules build _build_default _up _mod _constraints docs _docs _stubs _gen _sync check _check_default _scan _fmt _types _pyre _pol _cqrs _coordination coordination makefile status _status test _test_default val clean _clean_default ship _rel _pr _save _tag _push _imp _stat
...
boot: ## Bootstrap .venv + submodules (WHAT=all|venv|submodules|sync|stat|imp)
	$(call _run_workspace_hooks,pre,boot,$(WHAT))
	$(Q)$(FLEXT_MAKE_DISPATCH) boot
	$(call _run_workspace_hooks,post,boot,$(WHAT))

_boot_submodules: ## Initialize declared submodules without touching the environment
	$(Q)$(PREFLIGHT_CHECK)
	...
	$(Q)if [ -f .gitmodules ]; then \
		...
		$(ENSURE_CLEAN_SUBMODULE_WORKTREES); \
		...
		if ! git submodule update --init --recursive --jobs $${JOBS:-8} -- $$submodule_paths; then \
			echo "ERROR: failed to update declared submodules from .gitmodules"; \
...
_boot_venv: ## Repair the root workspace environment from the existing lock
	$(Q)$(PREFLIGHT_CHECK)
	$(Q)mkdir -p "$(UV_CACHE_DIR)"
	$(Q)flock -x "$(WORKSPACE_BOOT_LOCK)" uv sync --locked --all-packages --all-groups --all-extras --reinstall

_boot_default: ## Initialize submodules, repair .venv, then complete workspace setup
	$(Q)$(MAKE) --no-print-directory _boot_submodules
	$(Q)$(MAKE) --no-print-directory _boot_venv
	...
	$(Q)if [ -n "$(BOOT_VALIDATE_PROJECTS)" ]; then \
...
_constraints: ## Rewrite dependency constraints from uv.lock (policy=floor). Use PROJECT= or PROJECTS= to scope.
	...
	$(Q)$(ENSURE_SELECTED_PROJECTS)
	$(Q)$(ENSURE_PROJECTS_EXIST)
	$(Q)echo "Refreshing workspace dependency resolution..."; \
		log_file="/tmp/flext-constraints-lock.log"; \
		if uv lock --upgrade >"$$log_file" 2>&1; then \
			rm -f "$$log_file"; \
		else \
			cat "$$log_file"; \
...
check: ## Quality gates (WHAT=all|scan|fmt|format|types|pyre|pol|cqrs|lint|pyrefly|mypy|pyright|markdown|silent-failure|loc-cap|boundary|coordination)
	$(call _run_workspace_hooks,pre,check,$(WHAT))
	$(Q)$(FLEXT_MAKE_DISPATCH) check
	$(call _run_workspace_hooks,post,check,$(WHAT))
...
build: ## Build/regen (WHAT=all|gen|mod|up|constraints|sync|docs|stubs)
	$(call _run_workspace_hooks,pre,build,$(WHAT))
	$(Q)$(FLEXT_MAKE_DISPATCH) build
	$(call _run_workspace_hooks,post,build,$(WHAT))
...
ship: ## Release workflow (WHAT=all|save|tag|push|pr|rel)
	$(call _run_workspace_hooks,pre,ship,$(WHAT))
	$(Q)$(FLEXT_MAKE_DISPATCH) ship
	$(call _run_workspace_hooks,post,ship,$(WHAT))
...
docs: ## Run docs pipeline (DOCS_PHASE=audit|fix|build|generate|validate|all)
	$(call _run_workspace_hooks,pre,docs,$(WHAT))
	$(Q)$(FLEXT_MAKE_DISPATCH) docs
	$(call _run_workspace_hooks,post,docs,$(WHAT))

test: ## Run tests only in all projects
	$(call _run_workspace_hooks,pre,test,$(WHAT))
	$(Q)$(FLEXT_MAKE_DISPATCH) test
	$(call _run_workspace_hooks,post,test,$(WHAT))
...
val: ## Run validate gates (WHAT=all|project|workspace, VALIDATE_SCOPE=project|workspace, FIX=1)
	$(call _run_workspace_hooks,pre,val,$(WHAT))
	$(Q)$(FLEXT_MAKE_DISPATCH) val
	$(call _run_workspace_hooks,post,val,$(WHAT))
...
clean: ## Clean all projects
	$(call _run_workspace_hooks,pre,clean,$(WHAT))
	$(Q)$(FLEXT_MAKE_DISPATCH) clean
	$(call _run_workspace_hooks,post,clean,$(WHAT))
...
ifneq ($(HAS_EXPLICIT_PROJECT_SELECTION),)
	...
	$(Q)echo "==> Regenerating production and test-facade __init__.py files for selected projects..."
	$(Q)for proj in $(SELECTED_PROJECTS); do \
		$(WORKSPACE_INFRA_CODEGEN) init --workspace "$(CURDIR)/$$proj" --apply || exit 1; \
		if [ -d "$(CURDIR)/$$proj/tests" ]; then \
			$(WORKSPACE_INFRA_CODEGEN) init --workspace "$(CURDIR)/$$proj" --module tests --apply || exit 1; \
		fi; \
	done
else
	...
	$(Q)$(WORKSPACE_INFRA_WORKSPACE) sync \
		--workspace "$(CURDIR)" \
		--apply
	$(Q)echo "==> Regenerating production and test-facade __init__.py files..."
