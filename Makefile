...
WORKSPACE_INFRA_MAINTENANCE := $(WORKSPACE_FLEXT_INFRA) maintenance
WORKSPACE_INFRA_REFACTOR := $(WORKSPACE_FLEXT_INFRA) refactor
WORKSPACE_INFRA_RELEASE := $(WORKSPACE_FLEXT_INFRA) release
...
mod: ## Modernize pyproject.toml files (standardize configs without lock/install)
	...
	$(WORKSPACE_INFRA_DEPS) modernize --skip-check --apply $(if $(PROJECT),--project "$(PROJECT)",) $(if $(PROJECTS),--projects "$(PROJECTS)",); \
	echo ""
	$(Q)echo "Syncing dependency paths to workspace mode..."; \
	$(WORKSPACE_INFRA_DEPS) path-sync --mode auto --apply $(if $(PROJECT),--project "$(PROJECT)",) $(if $(PROJECTS),--projects "$(PROJECTS)",); \
	echo ""
