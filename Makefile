...
WORKSPACE_PROJECTS := $(shell \
	$(PYTHON_CMD) -c 'import pathlib, tomllib; \
root_pyproject = pathlib.Path("pyproject.toml"); \
if not root_pyproject.is_file(): \
    raise SystemExit(0); \
try: \
    payload = tomllib.loads(root_pyproject.read_text()); \
except Exception: \
    raise SystemExit(0); \
tool_payload = payload.get("tool", {}) if isinstance(payload, dict) else {}; \
flext_payload = tool_payload.get("flext", {}) if isinstance(tool_payload, dict) else {}; \
workspace_payload = flext_payload.get("workspace", {}) if isinstance(flext_payload, dict) else {}; \
members = workspace_payload.get("members", []) if isinstance(workspace_payload, dict) else []; \
if not isinstance(members, list) or not members: \
    uv_payload = tool_payload.get("uv", {}) if isinstance(tool_payload, dict) else {}; \
    uv_workspace = uv_payload.get("workspace", {}) if isinstance(uv_payload, dict) else {}; \
    members = uv_workspace.get("members", []) if isinstance(uv_workspace, dict) else []; \
if isinstance(members, list): \
    print(" ".join(str(item).strip() for item in members if str(item).strip()), end="")')
EXTERNAL_PROJECTS := $(shell \
	$(PYTHON_CMD) -c 'import json, pathlib, tomllib; \
root = pathlib.Path("."); \
root_payload = {}; \
root_pyproject = root / "pyproject.toml"; \
if root_pyproject.is_file(): \
    try: root_payload = tomllib.loads(root_pyproject.read_text()); \
    except Exception: root_payload = {}; \
tool_payload = root_payload.get("tool", {}) if isinstance(root_payload, dict) else {}; \
flext_payload = tool_payload.get("flext", {}) if isinstance(tool_payload, dict) else {}; \
workspace_payload = flext_payload.get("workspace", {}) if isinstance(flext_payload, dict) else {}; \
members = workspace_payload.get("members", []) if isinstance(workspace_payload, dict) else []; \
if not isinstance(members, list) or not members: \
    ...
    uv_workspace = uv_payload.get("workspace", {}) if isinstance(uv_payload, dict) else {}; \
    members = uv_workspace.get("members", []) if isinstance(uv_workspace, dict) else []; \
members = {str(item).strip() for item in members if str(item).strip()} if isinstance(members, list) else set(); \
excluded = set(); \
docs_config = root / "docs" / "config.json"; \
if docs_config.is_file(): \
    try: \
        docs_payload = json.loads(docs_config.read_text()); \
        scope_payload = docs_payload.get("scope", {}) if isinstance(docs_payload, dict) else {}; \
        excluded = {str(item).strip() for item in scope_payload.get("exclude_roots", []) if str(item).strip()} if isinstance(scope_payload, dict) else set(); \
    except Exception: \
        excluded = set(); \
def normalize_name(spec): \
    ...
    for char in base: \
        ...
        if token: \
            break; \
    return "".join(token).lower().replace("_", "-"); \
def dependency_names(payload): \
    names = set(); \
    project_payload = payload.get("project", {}) if isinstance(payload, dict) else {}; \
    if isinstance(project_payload, dict): \
        for item in project_payload.get("dependencies", []) or []: \
            dep_name = normalize_name(item); \
            if dep_name: names.add(dep_name); \
        optional_payload = project_payload.get("optional-dependencies", {}); \
        if isinstance(optional_payload, dict): \
            for group in optional_payload.values(): \
                if isinstance(group, list): \
                    for item in group: \
                        dep_name = normalize_name(item); \
                        if dep_name: names.add(dep_name); \
    groups_payload = payload.get("dependency-groups", {}) if isinstance(payload, dict) else {}; \
    if isinstance(groups_payload, dict): \
        for group in groups_payload.values(): \
            if isinstance(group, list): \
                for item in group: \
                    dep_name = normalize_name(item); \
                    if dep_name: names.add(dep_name); \
    tool_section = payload.get("tool", {}) if isinstance(payload, dict) else {}; \
    poetry_payload = tool_section.get("poetry", {}) if isinstance(tool_section, dict) else {}; \
    poetry_deps = poetry_payload.get("dependencies", {}) if isinstance(poetry_payload, dict) else {}; \
    if isinstance(poetry_deps, dict): \
        for key in poetry_deps: \
            dep_name = normalize_name(key); \
            if dep_name and dep_name != "python": names.add(dep_name); \
    return names; \
for child in sorted(root.iterdir(), key=lambda item: item.name): \
    if (not child.is_dir()) or child.name.startswith(".") or child.name == "cmd" or child.name in excluded or child.name in members: \
        continue; \
    pyproject = child / "pyproject.toml"; \
    if not pyproject.is_file(): \
        continue; \
    try: payload = tomllib.loads(pyproject.read_text()); \
    except Exception: \
        continue; \
    tool_section = payload.get("tool", {}) if isinstance(payload, dict) else {}; \
    child_flext = tool_section.get("flext", {}) if isinstance(tool_section, dict) else {}; \
    docs_payload = child_flext.get("docs", {}) if isinstance(child_flext, dict) else {}; \
    enabled = docs_payload.get("enabled", True) if isinstance(docs_payload, dict) else True; \
    if isinstance(enabled, bool) and (not enabled): \
        continue; \
    if "flext-core" in dependency_names(payload): \
...
mod: ## Modernize pyproject.toml files (standardize configs without lock/install)
	...
	$(WORKSPACE_INFRA_DEPS) modernize --apply $(SELECTED_PROJECT_FLAGS); \
	echo ""
	$(Q)echo "Syncing dependency paths to workspace mode..."; \
	$(WORKSPACE_INFRA_DEPS) path-sync --mode auto --apply $(SELECTED_PROJECT_FLAGS); \
	echo ""
