...
install-workspace: venv-check
	...
	for proj in flx flx-database-oracle flx-http-oracle-oic flx-http-oracle-wms algar-mig-oud gruponos-poc-oic-wms flx-adapter-example; do \
		if [ -d "$$proj" ]; then \
	...
	@. $(VENV_DIR)/bin/activate && cd $(WORKSPACE_ROOT) && \
		...
		$(VENV_BIN)/pip install -e algar-mig-oud/ && \
		$(VENV_BIN)/pip install -e gruponos-poc-oic-wms/ && \
		$(VENV_BIN)/pip install -e flx-adapter-example/
	@echo ""
	@echo "✅ Validando instalação..."
	@. $(VENV_DIR)/bin/activate && cd $(WORKSPACE_ROOT) && \
		$(VENV_BIN)/python -c "import flx, flx_database_oracle, flx_http_oracle_oic, flx_http_oracle_wms, algar_oud_mig, gn_oic_wms_db; print('✅ Todos os imports funcionando!')" || \
