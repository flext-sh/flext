...
for proj in $(ALL_PROJECTS); do \
	$(WORKSPACE_FLEXT_INFRA) workspace sync --workspace "$$proj" --canonical-root "$(CURDIR)" --apply >/dev/null || exit 1; \
done
endef
...
boot: ## Install all projects into workspace .venv
	...
	for proj in $(SELECTED_PROJECTS); do \
		if [ -d "$$proj" ] && [ -f "$$proj/pyproject.toml" ]; then \
			...
			if $(WORKSPACE_FLEXT_INFRA) deps internal-sync --workspace "$$proj" >>"$$log_file" 2>&1; then \
				:; \
			else \
				echo "     sync  ... failed"; \
				cat "$$log_file"; \
			...
			printf "     install ... "; \
			if uv sync --directory "$$proj" --all-groups --all-extras >>"$$log_file" 2>&1; then \
	...
	if ! $(WORKSPACE_FLEXT_INFRA) deps internal-sync --workspace . >"$$log_file" 2>&1; then \
		echo "     sync  ... failed"; \
		cat "$$log_file"; \
		failed=$$((failed + 1)); \
...
up: ## Upgrade Python dependencies to latest via Poetry
	...
	if [ "$(DEPS_REPORT)" != "0" ]; then \
		...
		$(WORKSPACE_FLEXT_INFRA) deps detect -q --no-fail; \
	fi
	$(Q)echo "Syncing GitHub workflow templates..."
	$(Q)$(WORKSPACE_FLEXT_INFRA) github workflows --workspace "$(CURDIR)" --apply --prune --report .reports/workflows/sync.json
...
mod: ## Modernize pyproject.toml files (standardize configs without lock/install)
	...
	$(WORKSPACE_FLEXT_INFRA) deps modernize --skip-check --apply $(if $(PROJECT),--project "$(PROJECT)",) $(if $(PROJECTS),--projects "$(PROJECTS)",); \
	echo ""
	$(Q)echo "Syncing dependency paths to workspace mode..."; \
	$(WORKSPACE_FLEXT_INFRA) deps path-sync --mode auto --apply $(if $(PROJECT),--project "$(PROJECT)",) $(if $(PROJECTS),--projects "$(PROJECTS)",); \
	echo ""
...
rel: ## Interactive workspace release orchestration
	$(Q)$(PREPARE_RUNTIME_SELECTED_PROJECTS)
	$(Q)$(WORKSPACE_FLEXT_INFRA) release \
		--workspace "$(CURDIR)" \
		--phase "$(RELEASE_PHASE)" \
...
pr: ## Manage pull requests for selected projects
	$(Q)$(PREPARE_RUNTIME_SELECTED_PROJECTS)
	$(Q)$(WORKSPACE_FLEXT_INFRA) github pr-workspace \
		--workspace "$(CURDIR)" \
...
ifeq ($(VALIDATE_SCOPE),workspace)
	...
	$(Q)$(WORKSPACE_FLEXT_INFRA) validate inventory --workspace .
	$(Q)$(WORKSPACE_FLEXT_INFRA) validate basemk-validate
	$(Q)$(WORKSPACE_FLEXT_INFRA) github lint --workspace . --report .reports/workflows/actionlint.json
	$(Q)$(WORKSPACE_FLEXT_INFRA) validate skill-validate --skill scripts-validation --mode strict
	$(Q)$(WORKSPACE_FLEXT_INFRA) validate skill-validate --skill rules-github --mode strict
	$(Q)$(WORKSPACE_FLEXT_INFRA) validate skill-validate --skill rules-docker --mode strict
	$(Q)$(WORKSPACE_FLEXT_INFRA) deps modernize --audit
...
types: ## Run typings supply-chain (stubgen + stub_supply_chain + dependency report). Use PROJECT= or PROJECTS= to scope.
	...
	rm -rf "$$tmp_dir"
	$(Q)$(WORKSPACE_FLEXT_INFRA) validate stub-validate \
		$(if $(PROJECT),--project $(PROJECT),$(if $(PROJECTS),$(addprefix --project ,$(PROJECTS)),--all))
	$(Q)if [ "$(DEPS_REPORT)" != "0" ]; then \
		$(WORKSPACE_FLEXT_INFRA) deps detect --typings -q --no-fail; \
	fi
...
pyre: ## Authoritative repo-wide pyrefly report + policy gate -> .reports/pyrefly + evidence
	...
	$(Q)set -o pipefail; \
	$(WORKSPACE_FLEXT_INFRA) check run \
		--gates pyrefly \
	...
	policy_violations=0; \
	echo "--- # type: ignore ---"; \
	$(WORKSPACE_FLEXT_INFRA) validate scan --workspace . \
...
stubs: ## Repo-wide stub supply-chain validation -> .sisyphus/evidence
	...
	$(Q)set -o pipefail; \
	$(WORKSPACE_FLEXT_INFRA) validate stub-validate --all \
