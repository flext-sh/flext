...
for proj in $(ALL_PROJECTS); do \
	$(WORKSPACE_INFRA_WORKSPACE) sync --workspace "$$proj" --workspace "$(CURDIR)" --apply >/dev/null || exit 1; \
done
endef
...
mod: ## Modernize pyproject.toml files (standardize configs without lock/install)
	...
	$(WORKSPACE_INFRA_DEPS) modernize --skip-check --apply $(if $(PROJECT),--projects "$(PROJECT)",) $(if $(PROJECTS),--projects "$(PROJECTS)",); \
	echo ""
