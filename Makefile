...
fmt: ## Run code formatting across all workspace projects (ruff/gofmt + markdownlint)
	...
	$(Q)go_files=$$(find $(WORKSPACE_SELECTED_ROOTS) -type f -name '*.go' ! -path '*/.git/*' ! -path '*/vendor/*' ! -path '*/.venv/*'); \
	if [ -n "$$go_files" ]; then \
	...
	$(Q)if [ "$(HAS_EXPLICIT_PROJECT_SELECTION)" = "1" ]; then \
		md_files=$$(find $(WORKSPACE_SELECTED_ROOTS) -type f -name '*.md' ! -path '*/.git/*' ! -path '*/.reports/*' ! -path '*/.venv/*' ! -path '*/vendor/*' ! -path '*/node_modules/*' ! -path '*/dist/*' ! -path '*/build/*'); \
	else \
...
ifneq ($(HAS_EXPLICIT_PROJECT_SELECTION),)
	...
	$(Q)for proj in $(SELECTED_PROJECTS); do \
		$(WORKSPACE_INFRA_CODEGEN) lazy-init --workspace "$(CURDIR)/$$proj" --apply || exit 1; \
	done
else
