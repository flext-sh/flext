...
if $(UV_BIN) sync --all-packages --all-groups --all-extras $(WORKSPACE_EDITABLE_REINSTALL_FLAGS) >>"$$log_file" 2>&1; then \
	:; \
else \
	cat "$$log_file"; \
	exit 1; \
...
_boot_default: ## Install all projects into workspace .venv
	...
	# mro-qc84 (fix-forward): ENSURE_WORKSPACE_RUNTIME already locks + syncs the
	# whole workspace (uv sync --all-packages); the previous per-project uv lock
	# loop and the duplicate root lock/sync were redundant and slow. One sync.
	$(Q)echo "Workspace .venv synchronized ($(words $(SELECTED_PROJECTS)) projects + root)."
