...
def run_command(cmd: str, env_file: str = ".env.test") -> None:
    ...
    project_dir = Path(__file__).parent / "client-b-poc-oic-wms"
...
def main() -> None:
    ...
    # Copy test env file
    env_test = Path("client-b-poc-oic-wms/.env.test")
    env_file = Path("client-b-poc-oic-wms/.env")
    ...
    cli_file = Path("client-b-poc-oic-wms/src/gn_oic_wms_db/cli.py")
    if cli_file.exists():
