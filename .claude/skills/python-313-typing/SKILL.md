...
- **Zero Tolerance for Hacks**: Prohibited use of `model_rebuild()`, `eval()`, `exec()`, `cast()`, and `inline imports`. `Any` and `t.RecursiveContainer` are TOTALLY FORBIDDEN in type annotations — use `t.*` contracts from `typings.py`.
