...
- Preserve autogenerated package `__init__.py` lazy export layout: keep `TYPE_CHECKING` for type-only symbols, keep `_LAZY_IMPORTS`, and keep module-level `__getattr__`.
- Do not rewrite autogenerated `__init__.py` into eager runtime imports from sibling modules; that breaks lazy loading and increases cycle risk.

> **Rule**: See `AGENTS.md` §3 Code Law for the normative `TYPE_CHECKING` policy.
