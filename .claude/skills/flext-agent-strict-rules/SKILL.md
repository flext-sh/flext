...
- **`t.RecursiveContainer` is TOTALLY FORBIDDEN**: Never use `t.RecursiveContainer` as a type annotation. Use explicit domain models and protocol contracts only.
...
- **Tests MUST follow the EXACT SAME rules as production code**: Test files are NOT exempt from ANY typing, Pydantic v2, r, p, or architectural rule. Test fixtures use `Field()`, typed models, `r[T]` returns. Test data uses `t.*` types. No "test-only" relaxation exists.
...
- **Required**: Use **simple runtime aliases only**: direct assignment to the facade class (e.g. `c = FlextConstants`, `m = FlextModels`, `r = r`, `t = FlextTypes`, `u = FlextUtilities`, `p = FlextProtocols`, `d = d`, `e = e`, `h = h`, `s = s`, `x = x`). No alias registry; no staticmethod layer for defining c, m, r, p, t, u, p, d, e, h, s, x.
...
12. **Tests = Production Discipline (AXIOMATIC)**: Tests MUST demonstrate the EXACT SAME strict typing, Pydantic v2, r, p, and architectural discipline as production code. No "test-only" relaxation of any rule.
