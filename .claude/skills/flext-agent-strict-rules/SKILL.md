...
- **`Any` is TOTALLY FORBIDDEN**: Never use `Any` in type annotations, function signatures, return types, variable types, examples, or generated code. Replace with `t.*` contracts from `typings.py` (`object`, `t.Scalar`, `t.ConfigMap`, `t.Dict`, `t.JsonValue`, etc.).
