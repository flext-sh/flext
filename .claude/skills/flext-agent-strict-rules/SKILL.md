...
- **`object` is TOTALLY FORBIDDEN**: Never use `object` as a type annotation. Use explicit domain models and protocol contracts only.
- **`dict[str, Any]` is TOTALLY FORBIDDEN**: Replace with explicit model contracts (for example `m.<Domain>.InputModel`, `m.<Domain>.OutputModel`) and protocol boundaries (`p.*`).
...
- **AXIOMATIC — typings.py ALIAS TABLE IS LOCKED — READ THIS BEFORE ANY EDIT TO typings.py**:
  ...
  **RECURSIVE → MUST use `type X = ...` (self-referential, NEVER with isinstance):**
  `GeneralValueType`, `Serializable`, `JsonValue`.
