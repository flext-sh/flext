...
def validate(ctx: click.Context) -> None:
    ...
    console.print("[yellow]Validating FLX CLI setup...[/yellow]\n")
...
def env(ctx: click.Context) -> None:
    ...
    if flx_vars:
        table = Table(title="FLX Environment Variables")
        ...
        console.print("[yellow]No FLX environment variables found[/yellow]")
...
def paths(ctx: click.Context) -> None:
    ...
    paths = {
        "Config Directory": Path.home() / ".flx",
        "Config File": get_config_path(),
        "Cache Directory": Path.home() / ".flx" / "cache",
        "Log Directory": Path.home() / ".flx" / "logs",
        "Token File": Path.home() / ".flx" / ".token",
    }

    table = Table(title="FLX CLI Paths")
