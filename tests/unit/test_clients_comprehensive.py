...
class TestFlextLdapClientComprehensive:
    ...
    def test_client_execute_async_method(self) -> None:
        ...
        assert isinstance(result, FlextResult)
        # Without connection, execute_async should fail
        assert result.is_failure
    ...
    def test_client_test_connection_with_invalid_config(
    ...
    def test_client_validate_connection_with_config(
        self, ldap_config: FlextLdapModels.ConnectionConfig
    ) -> None:
        ...
        assert isinstance(result, FlextResult)
        # Should succeed with valid config (even without actual connection)
    ...
    def test_client_create_user_from_entry_result_invalid(self) -> None:
        ...
        # Invalid entry data
        entry_data = {"dn": "invalid-dn", "attributes": {}}
    ...
    def test_client_build_user_attributes(self) -> None:
        ...
        attributes = client._build_user_attributes(user_data)
    ...
    def test_client_factory_integration(
        self, flext_factories: FlextTestsFactories
    ) -> None:
        """Test client integration with flext_tests factories."""
        # Create client using factory
        client = flext_factories.create_ldap_client()
        ...
        # Test configuration
        if client.config:
            assert isinstance(client.config, FlextLdapModels.ConnectionConfig)
    ...
    def test_client_error_handling_consistency(self) -> None:
        ...
        for method_name, args in methods_to_test:
            ...
            assert isinstance(result, FlextResult), (
                f"Method {method_name} should return FlextResult"
            )

            # All methods should return failure without proper connection/config
            assert result.is_failure, (
                f"Method {method_name} should fail without connection"
            )
    ...
    def test_client_type_consistency(self) -> None:
        ...
        assert callable(client.session_id)
    ...
    def test_client_with_flext_tests_integration(
        self, flext_factories: FlextTestsFactories
    ) -> None:
        """Test client integration with flext_tests library."""
        # Test creating client with factory
        client = flext_factories.create_ldap_client()

        assert isinstance(client, FlextLdapClient)

        # Test creating configuration with factory
        config = flext_factories.create_ldap_config()

        assert isinstance(config, FlextLdapModels.ConnectionConfig)

        # Test creating client with factory config
        client_with_config = FlextLdapClient(config=config)

        assert client_with_config.config is not None
        assert client_with_config.config.server == config.server
