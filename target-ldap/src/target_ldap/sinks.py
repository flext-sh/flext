...
class LDAPSink(Sink):
        ...
        try:
            ...
            if self.config.get("dry_run_mode", False):
                self._process_dry_run(
                    working_record, transformation_result, validation_result
                )
                return
    ...
    def _process_enterprise_batch(
        self, batch: list[dict[str, Any]], context: dict[str, Any]
    ) -> None:
        ...
        for record in batch:
            try:
                ...
                if self._error_count >= self._max_errors:
                    logger.error("Maximum error threshold reached in batch processing")
                    raise
