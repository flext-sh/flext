...
- **U18 — config/settings são a base SSOT de informação consumida PELAS facetas `c/t/p/m/u` — SEMPRE (inviolável; espelha /flext-law §2.0; pedido vivo 2026-07-11).** As facetas NUNCA re-derivam, hardcodam ou re-leem fontes (env, arquivos, defaults) que `config`/`settings` já possuem: importam a SSOT (`from <ns> import config, settings`) e leem `config.<Projeto>.*` / `settings.<Projeto>.*` (U2). Uma faceta que inventa cópia própria de um valor que a SSOT detém é defeito corrigido na origem. A direção de dependência é uma via só: facetas consomem config/settings; config/settings NUNCA consomem as facetas em runtime (pureza de fundação — importáveis por `c/t/p/m/u`, só pydantic + stdlib + flext direto).
