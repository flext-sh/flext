...
- **U14 — Python 3.13 + Pydantic 2-way + PEP estrito (inviolável).** Tipagem moderna apenas (generics builtin, `X | Y`, `type` statements, protocolos estruturais; nunca `Any`/`object`). Pydantic 2-way obrigatório em todo payload próprio: `model_validate(...)` na entrada, `model_dump(...)` na saída — o round-trip é o contrato; payload model-less (`dict`/`TypedDict`) em fronteira própria é proibido. PEP 8/257/420; stdlib moderna antes de third-party; docstrings Google-style onde adotado. Artefatos (código, comentários, docstrings, logs, identificadores, saída de templates `.j2`) SEMPRE em inglês — traduzir legado não-inglês na mesma edição que tocar a região. U2–U8 regem juntas e continuam invioláveis.
...
- **U17 — Facetas puras declaration-only + models só Pydantic 2-way (inviolável; espelha /flext-law §1.14; pedido vivo 2026-07-11).** `constants` (c), `typings` (t), `protocols` (p), `models` (m), `settings` e `config` são facetas PURAMENTE declarativas: PROIBIDO qualquer helper, função ou método, público ou privado (compilação de regex, fluent builder, `@property` accessor, static/class helper) — toda behavior vive só em `u`/utilities, `cli`, `api`, `base`, `services/*` (assinatura abstrata de Protocol com corpo `...` é declaração, não behavior). `models` é SÓ Pydantic 2-way: modelo DEFINIDO apenas na faceta `models` e em nenhum outro lugar, contém SÓ declaração de campos — ZERO métodos; o contrato é `model_validate`/`model_dump`/`model_dump_json`/`model_validate_json`; validadores custom (`@field_validator`/`@model_validator`) só em último caso extremo quando não há outra forma; valores derivados (percentuais, contagens) são computados pela factory em `u` e gravados como campos plain (o modelo fica sem behavior e o `model_dump` emite o valor); modelos frozen usam defaults imutáveis (`tuple`/`frozenset`/`MappingProxyType`, nunca `list`/`dict`). `dict`/`TypedDict`/`NamedTuple`/`dataclass`/`SimpleNamespace`/payload JSON-tipado são ESTRITAMENTE proibidos como estrutura de dado ou contrato em qualquer faceta/camada (U6) — substituir por modelo `m.*` Pydantic-2.
...
- **U18 — config/settings são a base SSOT de informação consumida PELAS facetas `c/t/p/m/u` — SEMPRE, SEM ATRAVESSADORES (inviolável; espelha /flext-law §2.0+§2.2+§2.3; pedido vivo 2026-07-11).** As facetas NUNCA re-derivam, hardcodam ou re-leem fontes (env, arquivos, defaults) que `config`/`settings` já possuem: a ÚNICA forma de acesso é `from <namespace> import config, settings` e o consumo DIRETO dos singletons namespaced (`config.<Projeto>.*` / `settings.<Projeto>.*`), que chegam com TODOS os dados do projeto e subprojetos JÁ CARREGADOS (U2). **ATRAVESSADORES SÃO PROIBIDOS E DEVEM SER 100% REMOVIDOS**: nenhuma função/método/propriedade intermediária que "atravessa" para chegar a config/settings (sem `config.X_config()`, sem getter de repasse, sem wrapper de acesso, sem subscript de dict como contrato) — o consumo é o acesso direto ao singleton, e qualquer atravessador existente é removido na origem no mesmo ciclo (U7). **OS DADOS DE CONFIG SÃO VALIDADOS POR `_models/config.py` (padrão de referência: `../cosmos-main/`)**: `_models/config.py` importa SÓ pydantic — namespace class com modelos nested por domínio (`frozen=True, extra="forbid"`) + `Root` (`frozen=True, extra="ignore"`); `_config.py` (subclasse de `FlextConfig`, `CONFIG_DIR = "config"`) valida as fatias YAML model-less via `Root.model_validate(...)` numa `@cached_property` tipada por PROTOCOLO (`p` sob TYPE_CHECKING, nunca `m` em anotação) e expõe o singleton congelado `config` no `__init__.py`; novo domínio de config = um modelo nested em `_models/config.py` + um campo validado no `Root`, nada mais. Uma faceta que inventa cópia própria de um valor que a SSOT detém é defeito corrigido na origem. Os singletons `config`/`settings` são compostos SOMENTE a partir do `config.py` e `settings.py` do projeto sobre subclasses privadas `_*/*.py` (`_constants/_models/_protocols/_utilities` de config/settings) — e esses módulos privados NUNCA importam `c/t/p/m/u` de forma direta em runtime (pureza de fundação; `TYPE_CHECKING`-only para afiar anotações). A direção de dependência é uma via só: facetas consomem config/settings; config/settings nunca consomem as facetas.
