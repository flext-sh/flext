...
- **Relevant generic ECC skills (this stack)**: beyond path-scoped `rules-*` skills, the most relevant generic ECC skills for FLEXT (Python 3.13 + Go) are `golang-patterns`, `golang-testing`, `fastapi-patterns`, and `hexagonal-architecture` (the Go `cmd/`+`pkg/{domain,infrastructure}` ports-and-adapters layout — distinct from the Python `flext-architecture-layers` skill). Load them on demand via the ECC plugin only when the change touches that surface. Cross-cutting daily skills (`cost-tracking`, `token-budget-advisor`, `repo-scan`, `error-handling`, `architecture-decision-records`, `git-workflow`) are distributed globally via `~/.ai-hub` — reference, do not copy here.
