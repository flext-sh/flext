...
- Use `gt sling` / `gt convoy` / `gt mol` / `gt done` / `gt handoff` for lane lifecycle.
...
- Lane lifecycle is managed only by Gas Town (`gt sling` / `gt convoy` / `gt mol` / `gt done` / `gt handoff`).
...
- Before `gt done`, lane evidence requires `gt hook status`, convoy state, and
  the full applicable Make validation boundary. Refinery merge evidence is
  recorded only after authorized integration.
