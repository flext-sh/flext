...
- **U4 — Zero config/settings intermediaries.** `ConfigProxy`, `SettingsProxy`, `config_access`, `settings_access`, `settings`, `settings`, forwarding getters/properties, importlib resolvers, pass-through wrappers, and compatibility aliases are forbidden and must be removed at the source. Consumers access only the direct namespaced singletons from U2. Config payload schemas are declaration-only Pydantic models owned by `_models/config.py`; the `config.py` / `settings.py` foundation and their private subclasses never import `c/t/p/m/u` at runtime.
- **U5 — Import direction `c → t → p → m → u` (runtime-forward; ADR-011).** Forward (índice maior consome menor) SEMPRE em runtime; reverse (índice menor importando maior) é PROIBIDO por completo — nem em runtime nem sob `TYPE_CHECKING`. Todo nome usado em anotação avaliada em runtime (campo Pydantic, atribuição anotada PEP 526, assinatura decorada por beartype, RHS de `type` PEP 695) é import runtime top-level. cross-project (consumer → base upstream) livre em runtime; leaf config/settings nunca importam a facade `c/t/p/m/u` do próprio projeto (nem runtime nem `TYPE_CHECKING`; schemas de config importam só Pydantic/stdlib). Sem `model_rebuild()`; sem lazy import ad-hoc (só o mapa PEP 562 da raiz é sancionado).
- **U6 — Typing estrito (ADR-011).** Nunca `Any`/`object`. Composto usa alias de `t` (`t.MappingOf[K,V]`, `t.SequenceOf[T]`, …). Nullable sempre explícito `T | None` (fora). Campos de DADOS / aninhados / compostos de um model são `m.*` concretos (validação+serialização recursiva Pydantic). Campos de colaborador/DI (serviços injetados, container) são `p.*` (base com `arbitrary_types_allowed=True`). Assinaturas de `u`/`services`/`api` tipam models por `p.*` (import runtime, `u → p` forward), passando a instância `m.*` concreta adiante. `p` NUNCA importa `m` (nem `TYPE_CHECKING`); usa `p.BaseModel`. `t` é vocabulário puro (só `c`+stdlib+`t`).
...
<!-- mro-wkii.17.26 (agent: codex) — U20 now scopes PEP 562 exclusively to the production package root. -->
- **U20 — MRO/OO, facade order, runtime-forward annotations, and root-only lazy public exports are universal invariants (inviolable; live operator order 2026-07-14; ADR-011).** Every project extends the canonical upstream aliases and owns nested namespaces in strict dependency order `c → t → p → m → u`; reverse edges are FORBIDDEN entirely (not runtime, not `TYPE_CHECKING`) — a needed higher-layer type means the artifact belongs at that higher layer. Every name in a runtime-evaluated annotation is a top-level runtime import. Public objects at the production package root are exported through its generated PEP 562 lazy map (the ONLY sanctioned lazy mechanism) plus matching declarations. Every other package initializer follows U22 and never installs lazy exports. Leaf code maximizes canonical namespaced aliases and never replaces this design with direct concrete imports or parallel facades. Generic lint/type defaults do not overrule the architecture. A diagnostic may be disabled globally only when a reproducible command proves that the specific tool cannot model this exact root MRO/lazy construct, no project-side correction exists, the code is listed in the closed canonical tooling SSOT with an inline rationale, and the setting is propagated to every repository. Such exceptions are never generalized to adjacent codes, files, or real defects; per-file ignore hints remain forbidden.
...
<!-- mro-wkii.17.26 (agent: codex) — U22 records recursive static initializer generation at every internal depth. -->
- **U22 — Thin domain facades, private responsibility modules, and generated internal exports are universal (inviolable; live operator order 2026-07-14).** Every public or composition module — including `c/t/p/m/u`, operational facades, `services`, codegen, refactor, dependency, validation, and tooling domains — is a thin MRO/composition facade only. A domain facade such as `_utilities/rope.py` imports and composes focused implementation mixins from its matching private package (`_utilities/_rope/*.py`); the same `<layer>/<domain>.py` plus `<layer>/_<domain>/*.py` shape applies whenever a module accumulates more than one responsibility. External consumers import only the facade, never its private parts. Every importable internal directory at every depth has one generated `__init__.py` containing the generated header, package docstring, future annotations import, explicit relative same-name re-exports of direct sibling public symbols (`from .module import Name as Name`), and a deterministic sorted literal `__all__: tuple[str, ...]`; a directory with no direct public symbol still emits `__all__ = ()`. Internal initializers never flatten descendant symbols, never use PEP 562, and are never empty/docstring-only. Generated PEP 562 lazy exports exist only at the production package root. `__unit__.py`, compatibility aliases, forwarding wrappers, duplicate implementations, and parallel old/new paths are forbidden. A decomposition updates every consumer and removes the superseded path atomically, with Rope-semantic dependency/SCC evidence plus `rg`/`sg` textual proof and continuous-green gates.

<!-- mro-wkii.17.26 (agent: codex) — U23 records the ordered clean-baseline gate and automated-refactor ownership ratified by the operator. -->
- **U23 — Ordered clean baseline and automated conformance are mandatory (inviolable; live operator order 2026-07-14).** A stabilization wave executes in this strict order: align Beads/ADRs/docs/skills; finish every already-started integration or merge by fix-forward and remove all index conflicts and conflict markers; reach zero Ruff and Ruff-format drift, zero Pyrefly/Mypy/Pyright errors and warnings, green fresh imports, and green honest pytest across the complete declared workspace; only then begin the next architectural refactor. Broad refactors are planned, patched, and validated by the single `flext-infra codegen conform` transaction in a temporary worktree. Rope opens and indexes the complete manifest-backed FLEXT workspace once; project mode is allowed only outside a declared workspace. `rg`, ast-grep, LSP, Scope, Refactor, and CRG are discovery/proposal sensors, never independent acceptance or direct live-tree writers. Every dangerous apply requires a pushed checkpoint, deterministic dry-run, patch check, import/breakage proof, all gates, and a second empty plan. A red or conflicted baseline blocks feature and architecture work rather than becoming accepted debt.
