...
- **AXIOMATIC** Compatibility aliases for types (`MyScalar = t.Scalar`, `ConfigDict = t.ConfigMap`, `AnyValue = object`, or any `X = t.Y` renaming) are TOTALLY FORBIDDEN. Reference `t.*` directly at every usage site. No indirection layers, no local re-exports, no renaming aliases for types. Not even "convenience" aliases.
