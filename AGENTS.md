...
<!-- mro-3o9s (agent: kimi) — U18 (config/settings como base SSOT consumida PELAS facetas) gravada por pedido vivo (2026-07-11); espelha /flext-law §2.0; não remover sem ADR + pedido do operador. -->
- **U18 — config/settings são a base SSOT de informação consumida PELAS facetas `c/t/p/m/u` — SEMPRE (inviolável; espelha /flext-law §2.0; pedido vivo 2026-07-11).** As facetas NUNCA re-derivam, hardcodam ou re-leem fontes (env, arquivos, defaults) que `config`/`settings` já possuem: a ÚNICA forma de acesso é `from <namespace> import config, settings` e o consumo dos singletons namespaced (`config.<Projeto>.*` / `settings.<Projeto>.*`), que chegam com TODOS os dados do projeto e subprojetos JÁ CARREGADOS (U2). Uma faceta que inventa cópia própria de um valor que a SSOT detém é defeito corrigido na origem. Os singletons `config`/`settings` são compostos SOMENTE a partir do `config.py` e `settings.py` do projeto sobre subclasses privadas `_*/*.py` (`_constants/_models/_protocols/_utilities` de config/settings) — e esses módulos privados NUNCA importam `c/t/p/m/u` de forma direta em runtime (pureza de fundação; `TYPE_CHECKING`-only para afiar anotações). A direção de dependência é uma via só: facetas consomem config/settings; config/settings nunca consomem as facetas.
