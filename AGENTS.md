...
# AGENTS.md — FLEXT Workspace Overlay

The ai-hub-managed block above owns universal conduct. This compact overlay owns
only FLEXT workspace invariants; procedures belong in the selected skills.

## North Star

FLEXT is a typed Python integration platform. Shared primitives and project
metadata originate in `flext-core`; every member package extends those public
contracts without reimplementing them.

## Read First

1. [`docs/GOVERNANCE.md`](docs/GOVERNANCE.md) routes each concern to its owner.
2. [`.agents/provider.toml`](.agents/provider.toml) declares provider identity,
   activation marker, router, exported skills, and codemod provider.
3. [`.agents/commands/flext-law.md`](.agents/commands/flext-law.md) is the short
   session entry point.
4. Load only the smallest on-demand skills selected by
   [`flext-context-routing`](.agents/skills/flext-context-routing/SKILL.md).

## Fundamental Rules

- The objective, owning domain declaration, validated configuration, and
  fundamental rule are sources of truth. Tests, snapshots, examples, checks,
  reports, and generated projections only validate or consume them.
- Use one owner per fact. Update the owner and every affected docs, skill, agent,
  catalog, and consumer pointer in the same change; otherwise verify those
  surfaces remain current.
- Remove superseded code, aliases, wrappers, rules, and paths atomically. No
  fallback, compatibility surface, suppression, or old-plus-new coexistence.
- Public package contracts use canonical facades and aliases
  (`c/t/p/m/u` plus the operational aliases owned by that package). Consumers
  import the public facade, never private implementation parts.
- Validate owned external payloads once into their canonical Pydantic model and
  preserve that object through protocol contracts. Open or foreign mappings are
  allowed only when the owning protocol explicitly declares them.
- Consume config and settings through their directly exported, validated
  namespaced singletons. Do not reread, proxy, forward, or rederive them.
- Fallible domain flow uses the Result contract declared by `flext-core`.
  Detailed operation selection belongs to
  [`lib-returns`](.agents/skills/lib-returns/SKILL.md).
- Static enforcement policy and structural codemods have separate owners.
  Codemod identity and paths come only from the provider referenced by
