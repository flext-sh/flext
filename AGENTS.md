...
- **Private Mixin Naming**: Classes under `_models/`, `_utilities/`, `_protocols/`, and similar private trees MUST keep the project prefix and append only the module concern (e.g. `FlextInfraUtilitiesImportNormalizer`, `TestsFlextDocker`).
...
- **Subdirectory Composition Only via MRO**: Private files in `_models/`, `_utilities/`, `_protocols/`, and similar trees define mixin classes only. The public facade composes them directly in its inheritance list. Manual flat wrapper nesting such as `class Docker(TestsFlextDocker): pass` inside the facade namespace is STRICTLY FORBIDDEN.
