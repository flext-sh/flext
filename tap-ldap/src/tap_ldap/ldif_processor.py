    ...
    def _parse_lines(self, lines: list[str], source_name: str) -> Iterator[LDIFEntry]:
        ...
        for line in lines:
            ...
            if continuation_line:
                self._process_line(
                    continuation_line, current_entry, line_number - 1, source_name
                )
                continuation_line = ""
            ...
            current_entry = self._process_line(
                line, current_entry, line_number, source_name
            )

        # Handle any remaining continuation line
        if continuation_line:
            self._process_line(
                continuation_line, current_entry, line_number, source_name
            )
