...
class LDIFStream(LDAPStream):
    ...
    schema: ClassVar[dict[str, Any]] = th.PropertiesList(
        th.Property(
            "dn", th.StringType, required=True, description="Distinguished Name"
        ),
        ...
        th.Property(
            "object_classes", th.ArrayType(th.StringType), description="Object Classes"
        ),
        ...
        th.Property(
            "controls", th.ArrayType(th.StringType), description="LDIF controls"
        ),
        th.Property(
            "validation_status", th.StringType, description="Entry validation status"
        ),
        th.Property(
        ...
        th.Property(
            "migration_batch", th.StringType, description="Migration batch identifier"
        ),
        th.Property(
            "hierarchy_level", th.IntegerType, description="DN hierarchy level"
        ),
    ...
    def get_records(
        ...
        migration_batch = self.config.get(
            "migration_batch", f"batch_{processing_timestamp.strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        for file_path in ldif_files:
            ...
            try:
                yield from self._process_ldif_file(
                    file_path, processing_timestamp, migration_batch
                )
    ...
    def _get_ldif_files(self) -> list[Path]:
        ...
        if ldif_directory:
            ...
            if directory.exists() and directory.is_dir():
                ...
                logger.info(
                    "Found %d LDIF files in directory: %s", len(found_files), directory
                )
            else:
    ...
    def _process_ldif_file(
        self, file_path: Path, processing_timestamp: datetime, migration_batch: str
    ) -> Iterable[dict[str, Any]]:
...
class LDIFAnalysisStream(LDAPStream):
    ...
    schema: ClassVar[dict[str, Any]] = th.PropertiesList(
        th.Property(
            "source_file", th.StringType, required=True, description="Source LDIF file"
        ),
        th.Property(
            "file_size_bytes", th.IntegerType, description="File size in bytes"
        ),
        th.Property(
            "total_entries", th.IntegerType, description="Total number of entries"
        ),
        th.Property("entry_types", th.ObjectType(), description="Count by entry type"),
        th.Property(
            "object_classes", th.ObjectType(), description="Count by object class"
        ),
        th.Property(
            "hierarchy_levels", th.ObjectType(), description="Count by hierarchy level"
        ),
        th.Property(
            "validation_summary",
            th.ObjectType(),
        ...
        th.Property(
            "attributes_summary", th.ObjectType(), description="Attribute usage summary"
        ),
        th.Property(
            "analysis_timestamp",
            th.DateTimeType,
    ...
    def _analyze_ldif_file(
        self, file_path: Path, analysis_timestamp: datetime
    ) -> dict[str, Any]:
        ...
        recommendations = self._generate_recommendations(
            entry_types, object_classes, total_entries, validation_errors
        )
