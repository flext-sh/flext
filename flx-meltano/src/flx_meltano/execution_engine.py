...
class MeltanoPipelineExecutor:
    ...
    async def execute_pipeline(
        self, pipeline_id: str, execution_request: PipelineExecutionRequest, user_id: str
    ...
    async def get_execution_status(self, execution_id: str, user_id: str) -> ServiceResult[PipelineExecutionResponse]:
            ...
            if execution_id in self._active_executions:
                ...
                if task.done():
                    ...
                    try:
                        ...
                        if result.success:
                            ...
                            message = result.error.message if result.error else "Pipeline execution failed"
                    except (
                        RuntimeError,
                        ValueError,
                        TypeError,
                ...
                return ServiceResult.ok(
                    PipelineExecutionResponse(
                        ...
                        completed_at=(
                            datetime.now(UTC) if status in {ExecutionStatus.COMPLETED, ExecutionStatus.FAILED} else None
                        ),
    ...
    async def cancel_execution(self, execution_id: str, user_id: str) -> ServiceResult[dict[str, str]]:
    ...
    async def _execute_pipeline_async(
        ...
        try:
            ...
            execution_config = ExecutionConfig(
                ...
                timeout_seconds=execution_request.timeout_seconds or self.constants.DEFAULT_PIPELINE_TIMEOUT_SECONDS,
            ...
            await self._update_execution_status(
                ...
                completed_at=datetime.now(UTC),
                message=(result.error.message if result.error else "Pipeline execution failed"),
                error_output=(result.error.details.get("stderr", "") if result.error else ""),
            )
    ...
    async def _get_pipeline_config(self, pipeline_id: str) -> ServiceResult[dict[str, Any]]:
    ...
    async def _create_job_execution(
        self, execution_id: str, pipeline_id: str, user_id: str, execution_request: PipelineExecutionRequest
    ) -> None:
    ...
    async def _build_meltano_command(
        self, pipeline_config: dict[str, Any], execution_request: PipelineExecutionRequest
    ) -> list[str]:
    ...
    async def _log_execution_output(self, execution_id: str, stream: str, line: str) -> None:
        """Log execution output for streaming and storage."""
        # TODO: Implement log storage and streaming

    async def _validate_execution_access(self, execution_id: str, user_id: str) -> ServiceResult[bool]:
