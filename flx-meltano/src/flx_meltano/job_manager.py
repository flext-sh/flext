...
class FlxMeltanoJobManager:
    ...
    async def list_jobs(
        self, project: Project, state: State | None = None, run_id: str | None = None, limit: int = 100, offset: int = 0
    ) -> list[Job]:
    ...
    async def get_job_statistics(self, project: Project, days: int = 7) -> dict[str, Any]:
        ...
        try:
            ...
            with session_factory() as session:
                # Get total job count
                total_jobs = session.query(Job).filter(Job.started_at >= cutoff_date).count()

                # Get job counts by state
                state_counts = {}
                for state in State:
                    count = session.query(Job).filter(Job.started_at >= cutoff_date).filter(Job.state == state).count()
    ...
    async def get_running_jobs(self, project: Project, heartbeat_timeout_minutes: int = 5) -> list[Job]:
    ...
    async def get_stale_jobs(self, project: Project, heartbeat_timeout_minutes: int = 5) -> list[Job]:
                ...
                result["would_clean"] = [
                    {
                        "job_id": job.job_id,
                        "run_id": job.run_id,
                        "last_heartbeat": (job.last_heartbeat_at.isoformat() if job.last_heartbeat_at else None),
                    }
                    for job in stale_jobs
                ]
            else:
    ...
    async def get_job_performance_metrics(self, project: Project, days: int = 30) -> dict[str, Any]:
            ...
            with session_factory() as session:
                ...
                for job in completed_jobs:
                    if job.started_at and job.last_heartbeat_at:
                        duration = (job.last_heartbeat_at - job.started_at).total_seconds()
                ...
                if durations:
                    ...
                    avg_duration = min_duration = max_duration = p50_duration = p95_duration = 0
                ...
                metrics = {
                    ...
                    "success_rate": round(
                        len([j for j in completed_jobs if j.state == State.SUCCESS]) / len(completed_jobs) * 100,
                        2,
                    ),
                    "generated_at": datetime.now(UTC).isoformat(),
                }
    ...
    async def delete_old_jobs(
        self, project: Project, days_to_keep: int = 30, execution_mode: JobExecutionMode = JobExecutionMode.DRY_RUN
    ) -> dict[str, Any]:
